...
async def test_save_message_persists_thread_summary_and_emits_it(
    ...
    async def _record_thread_event(event: object) -> None:
...
async def test_save_message_backfills_missing_summary_without_overwriting_existing(
    ...
    async def _ignore_thread_event(event: object) -> None:
        del event
        return None

    monkeypatch.setattr(module, "get_session", session_factory)
    emitter = DashboardEmitter(enabled=True)
    monkeypatch.setattr(emitter, "publish", _ignore_thread_event)
    set_dashboard_emitter(emitter)
