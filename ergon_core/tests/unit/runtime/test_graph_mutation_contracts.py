...
from ergon_core.core.application.samples.events import sample_runtime_event_from_row
...
def test_rest_and_dashboard_events_share_typed_sample_wal_payloads() -> None:
    ...
    assert event.event.table == "sample_edge_events"
    assert event.event.event_type == "edge.added"
    assert event.event.target_id == edge_id
    data = event.model_dump(mode="json")
    assert data["event"]["payload"]["source_task_id"] == str(source_id)
    assert data["event"]["payload"]["target_task_id"] == str(target_id)
