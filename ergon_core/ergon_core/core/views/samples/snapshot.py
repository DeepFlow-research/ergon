...
def _build_task_map(
    ...
    task_map: dict[str, SampleTaskDto] = {}
...
) -> dict[str, list[SampleResourceDto]]:
...
def _task_keyed_evaluations(
    evaluations: list[SampleTaskEvaluation],
    sample_id: str,
) -> dict[str, SampleTaskEvaluationDto]:
    result: dict[str, SampleTaskEvaluationDto] = {}
...
def _task_timestamps(
    ...
    by_task: dict[UUID, list[SampleTaskAttempt]] = defaultdict(list)
...
def _context_events_by_task(
    context_events_rows: list[SampleContextEvent],
    execution_task_map: dict[UUID, UUID],
