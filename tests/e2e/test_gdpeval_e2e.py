...
This test just waits for GDPEval runs to complete and asserts on results.
"""
...
class TestGDPEvalE2E:
    """
    Wait for GDPEval runs (already dispatched) and assert on results.
    """
    ...
    async def test_gdpeval_batch(self, all_dispatched_runs: AllDispatchedRuns, db_session: Session):
        ...
        print(f"\n📊 Results for {len(runs)} GDPEval runs:")
        ...
        assert len(failures) == 0, f"{len(failures)}/{len(runs)} GDPEval tasks failed"
