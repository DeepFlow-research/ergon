...
class TestRestartTaskBasic:
    ...
    async def test_restarts_from_terminal(self, session: Session, terminal_status: str):
        ...
        node = await _add_node(repo, session, run_id, "target", status=terminal_status)
    ...
    async def test_emits_task_ready(self, session: Session):
        ...
        node = await _add_node(repo, session, run_id, "target", status=COMPLETED)
    ...
    async def test_resets_outgoing_edges(self, session: Session):
    ...
    async def test_status_change_mutation_logged(self, session: Session):
    ...
    async def test_rejects_non_terminal(self, session: Session, live_status: str):
        """restart_task rejects PENDING and RUNNING nodes."""
        repo = WorkflowGraphRepository()
        svc = TaskManagementService(graph_repo=repo)
        run_id = uuid4()

        node = await _add_node(repo, session, run_id, "target", status=live_status)
    ...
    async def test_no_outgoing_edges_still_works(self, session: Session):
        ...
        node = await _add_node(repo, session, run_id, "leaf", status=FAILED)
...
async def _build_chain(repo, session, run_id, parent_id, keys: list[str], statuses: list[str]):
...
class TestDownstreamInvalidation:
    ...
    async def test_running_target_cancelled(self, session: Session):
        ...
        parent = await _add_node(repo, session, run_id, "manager", status=RUNNING)
        d, e = await _build_chain(
            repo, session, run_id, parent.id, ["D", "E"], [COMPLETED, RUNNING]
        )
    ...
    async def test_completed_target_cancelled_and_edges_reset(self, session: Session):
    ...
    async def test_deep_chain_cascades(self, session: Session):
    ...
    async def test_fan_in_invalidation_resets_both_incoming_edges(self, session: Session):
    ...
    async def test_already_cancelled_downstream_left_alone(self, session: Session):
        ...
        parent = await _add_node(repo, session, run_id, "manager", status=RUNNING)
        d, e = await _build_chain(
            repo, session, run_id, parent.id, ["D", "E"], [COMPLETED, CANCELLED]
        )
    ...
    async def test_failed_downstream_left_alone(self, session: Session):
        ...
        parent = await _add_node(repo, session, run_id, "manager", status=RUNNING)
        d, e = await _build_chain(repo, session, run_id, parent.id, ["D", "E"], [COMPLETED, FAILED])
