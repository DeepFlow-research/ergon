...
export type RunMetricExplorerMode = "1d" | "2d";
export type RunMetricExplorerView =
  | "empty"
  | "ranked-list"
  | "strip"
  | "histogram"
...
export function selectRunMetricExplorerView(
  mode: RunMetricExplorerMode,
  points: RunMetricPoint[],
  primaryMetricKey: RunMetricKey,
  secondaryMetricKey?: RunMetricKey,
): RunMetricExplorerView {
...
export function normalizeRunMetricPoint(run: ExperimentRunRow): RunMetricPoint {
  ...
  return {
    runId: run.run_id,
    runName: metrics.run_name ?? run.run_id.slice(0, 8),
    status: run.status,
