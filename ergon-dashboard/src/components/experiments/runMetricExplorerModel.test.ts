...
import {
  ...
  selectRunMetricExplorerView,
  type RunMetricPoint,
} from "./runMetricExplorerModel";
...
function point(index: number): RunMetricPoint {
  return normalizeRunMetricPoint(
    runRow({
      run_id: `${String(index).padStart(8, "0")}-2222-4222-8222-222222222222`,
...
test("selects small-n explorer views by mode and numeric threshold", () => {
  assert.equal(selectRunMetricExplorerView("1d", [point(1), point(2), point(3), point(4)], "score"), "ranked-list");
  assert.equal(selectRunMetricExplorerView("1d", Array.from({ length: 5 }, (_, index) => point(index + 1)), "score"), "strip");
  assert.equal(selectRunMetricExplorerView("1d", Array.from({ length: 20 }, (_, index) => point(index + 1)), "score"), "histogram");
  assert.equal(selectRunMetricExplorerView("2d", [point(1), point(2)], "score", "duration_ms"), "too-few-points");
  assert.equal(selectRunMetricExplorerView("2d", [point(1), point(2), point(3)], "score", "duration_ms"), "scatter");
});
