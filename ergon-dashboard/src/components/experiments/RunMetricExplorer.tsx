...
export function RunMetricExplorer({
...
}: RunMetricExplorerProps) {
  const [mode, setMode] = useState<RunMetricExplorerMode>(initialMode);
  ...
  const view = selectRunMetricExplorerView(mode, points, primaryMetricKey, secondaryMetricKey);
  const primaryDescriptor = metricDescriptor(primaryMetricKey);

  return (
    <section className="rounded-[var(--radius)] border border-[var(--line)] bg-[var(--card)] p-4 shadow-card" data-testid="run-metric-explorer">
