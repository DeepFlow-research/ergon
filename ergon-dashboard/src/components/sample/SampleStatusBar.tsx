...
/**
 * SampleStatusBar — segmented bar showing the count of leaf tasks in each
...
export interface SampleStatusBarProps {
...
export function SampleStatusBar({
  ...
  onFilter,
  activeFilter = null,
}: SampleStatusBarProps) {
  const safeTotal = total > 0 ? total : 1;

  return (
    <div className="flex flex-col gap-2" data-testid="sample-status-bar">
      ...
      <div
        ...
        data-testid="sample-status-counts"
      >
        {TASK_STATUS_ORDER.map((status) => {
          const tokens = STATUS_TOKENS[status];
          const count = counts[status] ?? 0;
