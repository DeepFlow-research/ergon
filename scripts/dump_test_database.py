...
def dump_failed_runs(session: Session, benchmark: BenchmarkName | None) -> str:
    ...
    if exp_ids:
        ...
        failed_runs = (
            list(session.exec(select(Run).where(Run.status == RunStatus.FAILED)).all())
            if not benchmark
            else []
        )
...
def dump_failed_actions(session: Session, benchmark: BenchmarkName | None) -> str:
    ...
    for action in failed_actions:
        ...
        lines.append(f"  started_at: {action.started_at}")
        lines.append(f"  run: {run if run else 'UNKNOWN'}")

        # Show input (truncated)
        if action.input:
            input_str = format_value(action.input, max_length=300)
...
def dump_failed_evaluations(session: Session, benchmark: BenchmarkName | None) -> str:
    ...
    for cr in failed_criteria:
        ...
        lines.append(
            f"  criterion: {cr.criterion_description[:100] if cr.criterion_description else 'NULL'}..."
        )
...
def dump_all_runs(session: Session, benchmark: BenchmarkName | None) -> str:
    ...
    for run in runs:
        ...
        lines.append(f"   status: {run.status.value}")
        lines.append(
            f"   final_score: {run.final_score if run.final_score is not None else 'NULL'}"
        )
        lines.append(
            f"   questions_asked: {run.questions_asked if run.questions_asked is not None else 'NULL'}"
        )
