...
async def main(file_path: str) -> ReadPDFResponse:
    ...
    try:
        ...
        if not file_path_obj.exists():
            return ReadPDFResponse(success=False, error=f"PDF file not found at {file_path}")
        ...
        if not pages:
            return ReadPDFResponse(success=False, error="No text could be extracted from the PDF")
