...
class LLMJudgeRule(BaseCriterion):
    ...
    async def evaluate(
        ...
        text_content = self._build_text_content(
            context.task_input, context.agent_reasoning, context.max_score
        )
        content_parts = self._build_content_parts(text_content, context.agent_outputs)
        messages = self._build_messages(content_parts)
        evaluation_input = str(messages)

        try:
            response = await runtime.call_llm_judge(messages, LLMJudgeResponse)
            score = context.max_score if response.final_verdict else 0.0
            feedback = response.reasoning
        except Exception as exc:
            feedback = f"Error in LLM judge evaluation: {exc}"
            score = 0.0
        ...
        return CriterionResult(
            ...
            evaluated_resource_ids=[str(r.id) for r in context.agent_outputs],
        )
