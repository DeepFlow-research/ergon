...
- **Leaking sandboxes on cancellation.** Cancellation cleanup must go through `cleanup_cancelled_task_fn`; writing `CANCELLED` directly to graph or execution rows bypasses both descendant cancellation and sandbox release.
