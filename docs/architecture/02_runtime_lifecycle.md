...
- **Dynamic subtask creation.** The graph is eagerly built for static DAGs at `workflow-start`. Beyond that, there are two sanctioned seams:
  1. **Manager toolkit path (legacy).** `SubtaskLifecycleToolkit.add_subtask` → `TaskManagementService.add_subtask(session, command: AddSubtaskCommand)` → `WorkflowGraphRepository.add_node` / `add_edge` (mutation kind `node.added`, actor `manager-worker`). The command carries `task_slug`, `description`, `assigned_worker_slug`, and `depends_on`; the new node has no `task_json` snapshot (defaults synthesised inside `add_node`).
  2. **Worker facade path (PR 9, graph-native).** `WorkerContext.spawn_task(task: Task, *, depends_on=())` → `TaskManagementService.spawn_dynamic_task(*, run_id, parent_task_id, task, depends_on)` → `WorkflowGraphRepository.add_node` with `task_json=task.model_dump(mode="json")` and `is_dynamic=True` (mutation kind `node.added`, actor `worker-context`). No `experiment_definition_tasks` row is written; the full Task snapshot lives in `run_graph_nodes.task_json` and re-inflates through `graph_repo.node` → `Task.from_definition`. Returns a `SpawnedTaskHandle(task_id=node.id)`.
  Both paths are append-only at the row level. A `TaskReadyEvent` is emitted by the manager path (no edge dependencies); the worker-facade path leaves dispatch to the surrounding worker flow. Workers must not touch the repository or a session directly — `WorkerContext.spawn_task` is the only sanctioned seam from worker code, and only that worker's descendants are reachable via `cancel_task` / `refine_task` / `restart_task` / `get_task` (containment is enforced by `WorkerContext._assert_descendant`, raising `ContainmentViolation`).
