...
| real-LLM | `tests/real_llm/` | Docker + Inngest + Postgres + Playwright; opt-in via `ERGON_REAL_LLM=1`; `OPENROUTER_API_KEY` only required for tests marked `real_llm_billing` (stub-only canary runs are zero-cost regardless of key presence); manual dispatch only (not in CI) |

### real-LLM tier

Optional pytest tier at `tests/real_llm/`, gated by `ERGON_REAL_LLM=1`, runs
real Sonnet-class experiments against a full local stack (backend + dashboard
+ Inngest + Postgres) via `docker-compose.real-llm.yml`. Assertions combine
Postgres state, `/api/test/*` harness endpoints, and Playwright dashboard
checks. An `OpenRouterBudget` session fixture gates total spend, and is
opt-in via the `real_llm_billing` pytest marker — tests that actually call
OpenRouter carry the marker; stub-only canary runs (e.g.
`tests/real_llm/benchmarks/test_smoke_stub.py`) are unmarked and remain
zero-cost regardless of whether `OPENROUTER_API_KEY` is present. This tier
is a bug-hunting instrument; it is not required for CI.
