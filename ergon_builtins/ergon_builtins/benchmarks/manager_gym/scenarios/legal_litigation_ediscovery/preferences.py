...
def create_preferences() -> PreferenceWeights:
                ...
                if indicator.lower() in str(res.content or "").lower():
                    if any(
                        (
                            ...
                            for resolution in ["resolved", "defended", "addressed", "remediated"]
                        )
                    ):
                        pressure_handled += 1
                    break
    ...
    def chain_of_custody_signal(workflow: Workflow) -> float:
        keywords = ("chain of custody", "hash", "sha256", "md5", "forensic image", "bitstream")
        hits = 0
        total = 0
        for res in workflow.resources.values():
            total += 1
    ...
    timeline: dict[int, PreferenceWeights] = {
        ...
        30: PreferenceWeights(
            preferences=[
                ...
                Preference(name="confidentiality", weight=0.2),
        ...
        45: PreferenceWeights(
            preferences=[
                ...
                Preference(name="speed", weight=0.1),
                Preference(name="cost", weight=0.2),
            ]
        ),
    }
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does defensible collection with chain of custody exist with: ESI collection documented, chain-of-custody maintained, forensic integrity preserved, and collection logs complete? Return true if defensible collection with chain of custody is established, false otherwise.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="privilege_review_qc_validated",
            llm_prompt="Does validated privilege review with QC exist with: privilege review completed, privileged documents identified and protected, QC sampling performed, and privilege log maintained? Return true if privilege review with QC is validated, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="frcp_compliant_productions",
            llm_prompt="Do FRCP-compliant productions exist with: production protocol specifications met, Bates numbering applied, load files generated, metadata fields included, and timely delivery achieved? Return true if productions are FRCP-compliant, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="tar_responsiveness_review_complete",
            llm_prompt="Does complete TAR responsiveness review exist with: technology-assisted review implemented, seed sets established, coding decisions tracked, responsiveness review completed, and quality metrics achieved? Return true if TAR responsiveness review is complete, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="eca_data_sizing_completed",
            llm_prompt="Does completed ECA data sizing exist with: early case assessment performed, data volume estimated, effort sizing completed, and case strategy informed? Return 10.0 if ECA data sizing is completed, 5.0 if it is mentioned but not completed,0.0 otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="processing_culling_denist_performed",
            llm_prompt="Does performed processing and culling exist with: data processing completed, deNIST filtering applied, deduplication performed, and date/keyword culling executed? Return true if processing and culling are performed, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="protective_order_compliance",
            llm_prompt="Does protective order compliance exist with: No PII identifiable in any outputs which are going to be shared publicly, no signs of any violation of protective order requirements, confidentiality maintained, and court order adherence confirmed? Return true if protective order compliance is achieved, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="custodian_interviews_source_identification",
            llm_prompt="Do custodian interviews and source identification exist with: key custodians interviewed, data sources identified and mapped, custodian data holdings documented, and source completeness confirmed? Return 8.0 if custodian interviews and source identification are complete, 4.0 if it is mentioned but not completed, 0.0 otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="search_terms_validation",
            llm_prompt="Does search terms validation exist with: search terms defined and tested, search strategy documented, hit rates analyzed, and search effectiveness confirmed? Return 7.0 if search terms validation is complete, 3.0 if it is mentioned but not completed, 0.0 otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="work_product_segregation",
            llm_prompt="Does work product segregation exist with: attorney work product identified and protected, counsel communications preserved and segregated, work product privilege maintained, and document classification secure? Return true if work product segregation is established, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="family_deduplication_handling",
            llm_prompt="Does family deduplication handling exist with: document families identified, family grouping maintained, deduplication family-aware, and family integrity preserved? Return true if family deduplication handling is proper, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="ocr_text_extraction_quality",
            llm_prompt="Is there any sign that documents recieved from other parties that had to be OCR'd show signs of detected duplicates, low quality, or other issues? Return 5.0 if there are no such issues, 0.0 otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="redaction_privilege_logging",
            llm_prompt="Are the documents recieved from other parties that had to be redaacted not identifiable as original documents (but no other issues)? Return 5.0 if there are no such issues, 0.0 otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="production_load_files_metadata",
            llm_prompt="Are the production load files and metadata properly generated and processed? Return 4.0 if there are no such issues, 0.0 otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="audit_trails_documentation",
            llm_prompt="Do audit trails and documentation exist with: process documentation maintained, audit trails preserved, workflow tracking active, and documentation standards met? Return true if audit trails and documentation are comprehensive, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="court_deadlines_met",
            llm_prompt="Are there signs of court deadlines being met, or of efforts being made to meet them? Return 3.0 if there are no signs of missed court dates, 0.0 otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
