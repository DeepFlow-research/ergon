...
def create_preferences() -> PreferenceWeights:
    ...
    sec_compliance_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Score 0–8 for material weakness identification and remediation planning: disclosure adequacy, remediation timeline, management assessment quality. Award partial credit with materiality rationale. Output numeric score [0, 8].",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="quiet_period_compliance",
            llm_prompt="Evaluate quiet period compliance strategy: gun-jumping avoidance, test-the-waters eligibility and execution, communication controls, legal pre-clearance. Award partial credit with citations. Output numeric score [0, 6].",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    ...
    cost_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Assess legal and professional service cost justification: scope appropriateness, fee reasonableness, value delivery, cost control measures. Output numeric score [0, 6].",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="underwriter_cost_optimization",
            llm_prompt="Evaluate underwriter and advisory cost optimization: fee negotiation effectiveness, service scope alignment, comparative market analysis. Output numeric score [0, 4].",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does accepted S-1 registration statement exist with: complete narrative sections (Business, Risk Factors, MD&A), Regulation S-X compliant financials, SEC acceptance for review confirmed, and ≤2 major comment letter cycles achieved? Return true if S-1 is accepted and progressing smoothly, false otherwise.",
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="pcaob_audited_financials_completed",
            llm_prompt="Do completed PCAOB-audited financial statements exist with: audit sign-offs secured, comfort letters available for underwriters, age-appropriate periods covered, and reconciliations documented? Return true if PCAOB audit is complete and ready, false otherwise.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="independent_board_committees_certified",
            llm_prompt="Do certified independent board committees exist with: independent directors appointed, audit/compensation committees formed, NYSE/Nasdaq standards met, and committee charters filed? Return true if board committees are certified and compliant, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="sox_controls_documentation_ready",
            llm_prompt="Does ready SOX controls documentation exist with: disclosure controls & procedures (SOX 302) documented, ICFR roadmap established, management certification rehearsed, and 404(b) readiness achieved if applicable? Return true if SOX controls documentation is ready, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="edgar_submission_workflows_tested",
            llm_prompt="Do tested EDGAR submission workflows exist with: EDGAR test submissions validated, submission procedures documented, error-free filing capability confirmed, and technical readiness verified? Return true if EDGAR workflows are tested and ready, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="legal_tax_opinions_filed",
            llm_prompt="Do filed legal and tax opinions exist with: Staff Legal Bulletin No. 19 compliance confirmed, legal opinions executed, tax opinions secured, and regulatory filing requirements met? Return true if legal and tax opinions are filed, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="listing_requirements_checklist_complete",
            llm_prompt="Does complete listing requirements checklist exist with: market value tests met, shareholder distribution validated, exchange approval correspondence secured, and auditor PCAOB registration confirmed? Return true if listing requirements checklist is complete, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="risk_disclosure_register_comprehensive",
            llm_prompt="Does comprehensive risk disclosure register exist with: operational/financial/legal/cyber risks inventoried, consistent mapping to S-1 risk factors, risk assessment completed, and disclosure accuracy validated? Return true if risk disclosure register is comprehensive, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="quiet_period_communications_plan",
            llm_prompt="Does quiet period communications plan exist with: counsel-approved strategy documented, test-the-waters capability (if EGC), Reg FD safeguards implemented, and post-pricing disclosure controls established? Return true if communications plan is operational and compliant, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="corporate_governance_package_complete",
            llm_prompt="Does complete corporate governance package exist with: governance policies documented, board structure established, committee charters approved, and Rule 10A-3 compliance mapped? Return true if corporate governance package is complete, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="internal_controls_framework_validated",
            llm_prompt="Does validated internal controls framework exist with: control procedures documented, testing completed, deficiencies remediated, and effectiveness demonstrated? Return true if internal controls framework is validated, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="investor_relations_playbook_ready",
            llm_prompt="Does ready investor relations playbook exist with: IR strategy documented, stakeholder communication plans prepared, analyst targeting completed, and post-IPO IR framework established? Return true if IR playbook is ready for deployment, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="executive_compensation_disclosure",
            llm_prompt="Does executive compensation disclosure exist with: compensation analysis completed, peer benchmarking documented, proxy disclosure prepared, and compensation committee approval secured? Return true if executive compensation disclosure is complete, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="comfort_letter_request_lists",
            llm_prompt="Do comfort letter request lists exist with: underwriter requirements documented, auditor coordination completed, comfort letter procedures established, and closing logistics planned? Return true if comfort letter processes are ready, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="non_gaap_disclosure_controls",
            llm_prompt="Do non-GAAP disclosure controls exist with: non-GAAP measures defined, reconciliation procedures documented, SEC compliance validated, and disclosure consistency maintained? Return true if non-GAAP disclosure controls are established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="legal_proceedings_documentation",
            llm_prompt="Does legal proceedings documentation exist with: material litigation identified, disclosure requirements assessed, legal risk evaluation completed, and S-1 disclosure prepared? Return true if legal proceedings documentation is complete, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="board_meeting_minutes_documented",
            llm_prompt="Do documented board meeting minutes exist with: IPO authorization resolutions passed, key decisions documented, governance compliance recorded, and audit trail maintained? Return true if board meeting documentation is complete, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_correspondence_archive",
            llm_prompt="Does regulatory correspondence archive exist with: SEC communications logged, comment letter responses tracked, regulator feedback documented, and compliance history maintained? Return true if regulatory correspondence is properly archived, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
