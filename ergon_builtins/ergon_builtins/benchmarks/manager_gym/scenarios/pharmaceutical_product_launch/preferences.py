...
def create_preferences() -> PreferenceWeights:
                ...
                if indicator.lower() in str(res.content or "").lower():
                    if any(
                        (
                            ...
                            for resolution in ["resolved", "mitigated", "addressed", "corrected"]
                        )
                    ):
                        pressure_handled += 1
                    break
    ...
    def regulatory_compliance_indicators(workflow: Workflow) -> float:
        ...
        keywords = ("ectd", "ich", "fda", "ema", "cgmp", "validation", "dossier", "regulatory")
        total = 0
        hits = 0
        for res in workflow.resources.values():
            total += 1
    ...
    def patient_safety_progress(workflow: Workflow) -> float:
        ...
        safety_tasks = [
            ...
            for t in workflow.tasks.values()
            if any(
                (keyword in t.name.lower() for keyword in ["safety", "pharmacovigilance", "risk"])
            )
        ]
        if not safety_tasks:
            return 0.5
        completed_safety = len([t for t in safety_tasks if t.status.value == "completed"])
    ...
    regulatory_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Score 0–6 for multi-jurisdictional coordination: FDA/EMA alignment, local authority submissions, regulatory strategy harmonization across regions. Award partial credit with coordination evidence.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    manufacturing_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Assess supply chain and distribution qualification: logistics partner validation, cold-chain integrity, serialization implementation, and distribution readiness. Award partial credit with qualification evidence. Output numeric score [0, 6].",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    ...
    speed_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Assess regulatory timeline optimization and efficiency: critical path management, parallel processing utilization, regulatory milestone achievement. Output numeric score [0, 6].",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="manufacturing_timeline_efficiency",
            llm_prompt="Evaluate manufacturing validation timeline efficiency: equipment qualification speed, validation batch execution, analytical method validation pace. Output numeric score [0, 4].",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    cost_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Assess regulatory and development cost optimization: consultant utilization efficiency, regulatory fee management, development cost control. Output numeric score [0, 4].",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does submitted eCTD regulatory dossier exist with: complete modules (quality, safety, nonclinical, clinical), FDA/EMA submission confirmed, ICH guidelines (Q8-Q11, M4) compliance, and formal agency acceptance received? Return 20.0 if there is evidence of an eCTD dossier being submitted and accepted, 10.0 if there is evidence of any of the following: eCTD dossier being submitted but it is not complete, FDA/EMA submission is planned and completed but not confirmed, ICH guidelines (Q8-Q11, M4) are shown to be compliant, formal agency acceptance not received, 0.0 otherwise.",
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cgmp_manufacturing_validated",
            llm_prompt="Does validated cGMP manufacturing exist with: external audit reports completed, IQ/OQ/PQ equipment qualification documented, lot release procedures validated, and no unresolved major observations? Return 18.0 if there is evidence of cGMP manufacturing being validated and audit-ready, 10.0 if there is evidence of any of the following: external audit reports completed but not yet received, IQ/OQ/PQ equipment qualification documented but not yet received, lot release procedures validated but not yet received, unresolved major observations, 0.0 otherwise.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="qbd_framework_documented",
            llm_prompt="Does documented QbD (Quality by Design) framework exist with: critical quality attributes (CQAs) defined, critical process parameters (CPPs) established, control strategy implemented, and reproducibility demonstrated across validation batches? Return 15.0 if there is evidence of a QbD framework being documented and validated, 10.0 if there is evidence of any of the following: critical quality attributes (CQAs) defined but not yet received, critical process parameters (CPPs) established, control strategy implemented, reproducibility demonstrated across validation batches, 0.0 otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="pharmacovigilance_system_operational",
            llm_prompt="Does operational pharmacovigilance system exist with: safety management plan active, adverse event reporting pathways established, signal detection protocols implemented, and risk minimization measures documented? Return 15.0 if there is evidence of a pharmacovigilance system being operational with all conditions met, 10.0 if there is evidence of any of the following: safety management plan active, adverse event reporting pathways established, signal detection protocols implemented, risk minimization measures documented, 0.0 otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="plair_commercial_authorization",
            llm_prompt="Does PLAIR (Pre-Launch Activities Importation Request) authorization exist with: FDA/EMA commercial staging approval, product import clearance, and commercial release readiness confirmed? Return 12.0 if there is evidence of a PLAIR authorization being secured, 10.0 if there is evidence of any of the following: FDA/EMA commercial staging approval, product import clearance, commercial release readiness confirmed, 0.0 otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="market_access_strategy_approved",
            llm_prompt="Does approved market access strategy exist with: payer dossiers completed, HTA submissions filed, pricing models validated, and early access programs operational? Return 12.0 if there is evidence of a market access strategy being approved, 5.0 if there is evidence of any of the following: payer dossiers completed, HTA submissions filed, pricing models validated, early access programs operational, 0.0 otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="supply_chain_readiness_validated",
            llm_prompt="Does validated supply chain readiness exist with: logistics partners qualified, cold-chain testing completed, serialization/track-and-trace compliance operational, and inventory ramp-up executed? Return 10.0 if there is evidence of supply chain being validated and ready, 5.0 if there is evidence of any of the following: logistics partners qualified, cold-chain testing completed, serialization/track-and-trace compliance operational, inventory ramp-up executed, 0.0 otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="governance_sign_offs_secured",
            llm_prompt="Do secured governance sign-offs exist with: Regulatory Affairs approval, Quality approval, Pharmacovigilance approval, Commercial approval, and Executive Board authorization documented? Return true if all governance sign-offs are secured, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_correspondence_documented",
            llm_prompt="Does documented regulatory correspondence exist with: FDA/EMA communication logs, agency feedback responses, deficiency resolution tracking, and first cycle review progress confirmed? Return true if regulatory correspondence is documented, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="clinical_data_package_complete",
            llm_prompt="Does complete clinical data package exist with: efficacy endpoints met, safety profile characterized, statistical analysis plan executed, and clinical study report finalized? Return 8.0 if clinical data package is complete, 3.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="analytical_method_validation",
            llm_prompt="Does analytical method validation exist with: assay validation completed, method transfer documented, stability studies finalized, and release testing protocols established? Return 8.0 if analytical methods are validated, 3.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="lifecycle_monitoring_plan",
            llm_prompt="Does lifecycle monitoring plan exist with: post-market surveillance strategy, continued process verification protocols, change control procedures, and lifecycle stage gates defined? Return 6.0 if lifecycle monitoring plan is operational, 3.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="decision_logs_maintained",
            llm_prompt="Do maintained decision logs exist with: launch readiness reviews documented, critical decision rationales captured, risk-benefit assessments recorded, and accountability trails established? Return 5.0 if decision logs are maintained, 3.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="batch_release_procedures",
            llm_prompt="Do batch release procedures exist with: release testing protocols validated, certificate of analysis templates approved, batch disposition criteria established, and quality person authorization documented? Return 4.0 if batch release procedures are established, 2.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="launch_readiness_assessment",
            llm_prompt="Does launch readiness assessment exist with: cross-functional readiness confirmed, go/no-go criteria evaluated, launch timeline validated, and stakeholder alignment documented? Return 4.0 if launch readiness assessment is complete, 2.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_agency_feedback_incorporation",
            llm_prompt="Does regulatory agency feedback incorporation exist with: FDA/EMA guidance integrated, comment responses submitted, deficiency remediation completed, and approval pathway confirmed? Return 3.0 if agency feedback is incorporated, 1.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
