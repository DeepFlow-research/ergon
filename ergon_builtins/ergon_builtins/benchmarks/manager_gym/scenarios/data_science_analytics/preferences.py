...
def create_preferences() -> PreferenceWeights:
    ...
    def experiment_tracking_signal(workflow: Workflow) -> float:
        ...
        keywords = ("mlflow", "wandb", "weights & biases", "comet", "experiment id", "run id")
        total = 0
        hits = 0
        for res in workflow.resources.values():
            total += 1
    ...
    def dq_test_density(workflow: Workflow) -> float:
        ...
        keywords = ("test", "assert", "expectation", "validation", "great expectations", "dq check")
        total = 0
        hits = 0
        for res in workflow.resources.values():
            total += 1
        ...
        WorkflowRubric(
            ...
            evaluator_function=lambda w: (
                lambda budget, actual: (
                    0.5
                    if budget <= 0
                    else 1.0 / (1.0 + max(0.0, (actual - budget) / max(1e-06, budget)))
                )
        ...
        WorkflowRubric(
            ...
            evaluator_function=lambda w: (
                lambda planned_avg, actual_avg: (
                    0.5
                    if planned_avg <= 0
                    else 1.0 / (1.0 + abs(actual_avg / max(1e-06, planned_avg) - 1.0))
                )
            )(
                w.total_budget / max(1, len(w.tasks)) if len(w.tasks) > 0 else 0.0,
                w.total_cost
                / max(1, len([t for t in w.tasks.values() if t.status.value == "completed"]))
                if len([t for t in w.tasks.values() if t.status.value == "completed"]) > 0
                else 0.0,
            ),
            max_score=1.0,
            run_condition=RunCondition.EACH_TIMESTEP,
    ...
    timeline: dict[int, PreferenceWeights] = {
        0: PreferenceWeights(
            preferences=[
                ...
                Preference(name="mlops", weight=0.1),
                Preference(name="data_governance", weight=0.15),
                Preference(name="business_value", weight=0.1),
        ...
        10: PreferenceWeights(
            preferences=[
                ...
                Preference(name="data_governance", weight=0.1),
                Preference(name="business_value", weight=0.1),
        ...
        60: PreferenceWeights(
            preferences=[
                ...
                Preference(name="mlops", weight=0.2),
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does deployed production-ready model exist with: CI/CD pipeline operational, model artifacts versioned and tracked, deployment package complete, and monitoring systems active? Return 18.0 if all conditions are met, removing 4 for every condition not met to a minimum of 0.0. Return 5.0 if there is evidence of any of these being planned but not yet completed.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="data_privacy_compliance_verified",
            llm_prompt="Does verified data privacy compliance exist with: PII policy compliance confirmed, secrets removed from artifacts, data access controls implemented, and privacy review passed? Return true is there is signs of an effort to verify data privacy compliance, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="reproducible_training_pipeline",
            llm_prompt="Does reproducible training pipeline exist with: fixed seeds implemented, environment captured and documented, experiment tracking active, and runs reproducible across environments? Return 15.0 if there is evidence of: 1. seeding, 2. environment pinning, 3. experiment tracking, removing 4.0 for each missing element. To a minimum of 0.0.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="bias_fairness_analysis_completed",
            llm_prompt="Does completed bias/fairness analysis exist with: bias metrics within policy thresholds, fairness analysis documented, mitigation strategies implemented, and responsible AI controls active? Return 12.0 if there is evidence of: 1. bias metrics within policy thresholds, 2. fairness analysis documented, 3. mitigation strategies implemented, 4. responsible AI controls active, removing 3.0 for each missing element. To a minimum of 0.0.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="model_performance_thresholds_met",
            llm_prompt="Do met model performance thresholds exist with: evaluation metrics (AUC/accuracy) above minimum thresholds, calibration validated, performance benchmarks achieved, and quality gates passed? Return 10.0 if there is evidence of: 1. evaluation metrics (AUC/accuracy) above minimum thresholds, 2. calibration validated, 3. performance benchmarks achieved, 4. quality gates passed, removing 2.0 for each missing element. To a minimum of 0.0.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="curated_dataset_with_lineage",
            llm_prompt="Does curated dataset with lineage exist with: data quality checks implemented, data lineage documented, governance controls applied, and dataset versioning operational? Return 10.0 if there is evidence of: 1. data quality checks implemented, 2. data lineage documented, 3. governance controls applied, 4. dataset versioning operational, removing 2.0 for each missing element. To a minimum of 0.0.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="model_card_and_explainability",
            llm_prompt="Do model card and explainability exist with: model card completed with performance metrics, explainability report generated, interpretability features documented, and transparency requirements met? Return true if model card and explainability are complete, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="deployment_readiness_gate_passed",
            llm_prompt="Does passed deployment readiness gate exist with: security review completed, rollback plan documented, monitoring strategy validated, and operational readiness confirmed? Return 7.0 if there is evidence of: 1. security review completed, 2. rollback plan documented, 3. monitoring strategy validated, 4. operational readiness confirmed, removing 1.0 for each missing element. To a minimum of 0.0.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="experiment_tracking_system",
            llm_prompt="Does experiment tracking system exist with: all experiments logged, hyperparameters tracked, model versions managed, and reproducibility ensured? Return true if experiment tracking system has been created, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="model_monitoring_alerts",
            llm_prompt="Do model monitoring alerts exist with: drift detection implemented, performance monitoring active, alert thresholds configured, and incident response procedures defined? Return 5.0 if model monitoring alerts are implemented, 3.0 if it is planned or partially complete but not finalized, 0.0 otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="business_requirements_documented",
            llm_prompt="Do documented business requirements exist with: success metrics defined, business objectives captured, stakeholder requirements documented, and acceptance criteria established? Return true if business requirements are documented, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="model_validation_framework",
            llm_prompt="Does model validation framework exist with: validation strategies implemented, cross-validation performed, holdout testing completed, and validation results documented? Return true if model validation framework is established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="data_access_governance",
            llm_prompt="Does data access governance exist with: access controls implemented, approval workflows established, audit trails maintained, and compliance monitoring active? Return true if data access governance is operational, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="project_documentation_complete",
            llm_prompt="Does complete project documentation exist with: methodology documented, assumptions captured, limitations identified, and knowledge transfer materials prepared? Return true if project documentation is complete, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
