...
from ergon_builtins.benchmarks.manager_gym.source_types import Workflow, TaskStatus
...
def _require_external_validation(workflow: Workflow, validation_keywords: List[str]) -> float:
    ...
    return min(1.0, validation_evidence / max(1, total_tasks * 0.3))
            ...
            if indicator.lower() in str(res.content or "").lower():
                if any(
                    (
                        ...
                        for resolution in ["resolved", "mitigated", "addressed", "negotiated"]
                    )
                ):
                    pressure_handled += 1
                break
    return min(1.0, pressure_handled / max(1, len(pressure_indicators) * 0.4))
...
compliance_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate compliance posture of the RIF: Does the plan respect EU collective redundancy rules and UK s.188 (consultation and authority notifications), and US WARN/mini‑WARN deadlines? Check that consultation occurs before issuing notices where required, and that statutory severance/benefits overlays are applied. Return [0, 10].",
        max_score=10.0,
...
employee_exp_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate the employee experience: clarity and empathy of manager scripts and letters, availability of outplacement/EAP support, and respect in day‑of sequencing. Return [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
docs_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Assess documentation/audit trail: decision logs, consultation minutes and responses, notice copies, authority filings, and post‑action audit completeness. Return [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does multi-jurisdictional legal compliance exist with: US WARN/mini-WARN compliance confirmed, EU collective consultation completed, UK TULRCA s.188 requirements met, FR CSE obligations satisfied, and all jurisdictional requirements addressed? Return true if legal compliance is multi-jurisdictional and complete, false otherwise.",
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="selection_criteria_defensible_documented",
            llm_prompt="Do documented defensible selection criteria exist with: objective selection criteria established, adverse-impact analysis performed, bias assessment completed, and legal defensibility confirmed? Return true if selection criteria are defensible and documented, false otherwise.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="privilege_strategy_confidentiality_maintained",
            llm_prompt="Does maintained privilege strategy and confidentiality exist with: attorney-client privilege preserved, work product protection maintained, confidential communications secured, and legal strategy protected? Return true if privilege strategy and confidentiality are maintained, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="governance_approval_accountability",
            llm_prompt="Does governance approval and accountability exist with: executive decision-making documented, board approval secured where required, accountability trails established, and governance compliance confirmed? Return true if governance approval and accountability are established, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="redeployment_mitigation_efforts_documented",
            llm_prompt="Do documented redeployment and mitigation efforts exist with: redeployment opportunities explored, alternative placement attempts documented, mitigation strategies implemented, and effort comprehensiveness demonstrated? Return true if redeployment and mitigation efforts are documented, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="employee_communications_executed",
            llm_prompt="Does executed employee communications exist with: employee notification strategy implemented, communication timing appropriate, messaging consistent and compassionate, and employee support provided? Return true if employee communications are executed effectively, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="benefits_payroll_transitions_complete",
            llm_prompt="Do complete benefits and payroll transitions exist with: severance packages processed, benefits transitions managed, COBRA/healthcare continuity arranged, and payroll changes executed? Return true if benefits and payroll transitions are complete, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulator_filings_notifications_timely",
            llm_prompt="Do timely regulator filings and notifications exist with: WARN notices filed, government notifications submitted, regulatory compliance maintained, and filing deadlines met? Return true if regulator filings and notifications are timely, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="post_action_audit_lessons_learned",
            llm_prompt="Do post-action audit and lessons learned exist with: implementation review completed, lessons learned documented, process improvements identified, and audit recommendations captured? Return true if post-action audit and lessons learned are complete, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="workforce_planning_analytics",
            llm_prompt="Does workforce planning analytics exist with: headcount analysis completed, skills gap assessment performed, organizational design optimized, and workforce planning strategic? Return true if workforce planning analytics are comprehensive, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="employee_support_services",
            llm_prompt="Do employee support services exist with: outplacement services provided, career transition support available, counseling resources offered, and employee assistance comprehensive? Return true if employee support services are adequate, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="stakeholder_communication_management",
            llm_prompt="Does stakeholder communication management exist with: internal stakeholder communication coordinated, external stakeholder management active, investor relations maintained, and communication consistency achieved? Return true if stakeholder communication management is effective, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="legal_risk_mitigation",
            llm_prompt="Does legal risk mitigation exist with: legal risks identified and assessed, risk mitigation strategies implemented, legal exposure minimized, and defensive posture strengthened? Return true if legal risk mitigation is effective, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="union_works_council_engagement",
            llm_prompt="Does union and works council engagement exist with: union consultation conducted, works council engagement completed, collective bargaining obligations met, and labor relations managed? Return true if union and works council engagement is proper, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="documentation_version_control",
            llm_prompt="Does documentation version control exist with: document management systematic, version control maintained, approval workflows established, and documentation integrity preserved? Return true if documentation version control is established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="change_management_support",
            llm_prompt="Does change management support exist with: change management strategy implemented, transition support provided, organizational change facilitated, and change adaptation supported? Return true if change management support is effective, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="business_continuity_maintained",
            llm_prompt="Does maintained business continuity exist with: operational continuity preserved, business disruption minimized, critical functions maintained, and service delivery sustained? Return true if business continuity is maintained, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cost_benefit_analysis_validated",
            llm_prompt="Does validated cost-benefit analysis exist with: financial analysis completed, cost savings quantified, business case validated, and ROI demonstration provided? Return true if cost-benefit analysis is validated, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
