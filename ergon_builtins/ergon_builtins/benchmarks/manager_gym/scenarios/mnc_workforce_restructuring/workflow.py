...
def create_workflow() -> Workflow:
    wf = Workflow(
        ...
        workflow_goal="Plan and execute a defensible, humane global workforce restructuring. Objectives: establish governance, define selection criteria, run legal/jurisdictional scoping (US WARN/mini‑WARN, EU collective consultation, UK TULRCA s.188, FR CSE), perform adverse‑impact analysis, redeployment/mitigation, prepare documentation and notices, execute communications and payroll/benefits changes, and complete post‑action audit and regulator filings.",
        owner_id=uuid4(),
    )
    program_charter = Task(
        name="Program Charter & Governance",
        description="Define objectives, scope, roles, and cadence; confirm privilege strategy with Legal; stand up risk/issue controls and a single source of truth (SSOT) for artifacts.",
    ...
    data_pull = Task(
        ...
        description="Compile HRIS roster, roles/locations, tenure, performance data, compensations/benefits, visas, and union/Works Council coverage; ensure data quality and minimization.",
        status=TaskStatus.PENDING,
    ...
    selection_criteria = Task(
        ...
        description="Define role‑based selection criteria (skills, redundancy of role, performance bands) and guard against discriminatory impact; set documentation templates and sign‑off path.",
        status=TaskStatus.PENDING,
    ...
    jurisdiction_scoping = Task(
        ...
        description="Identify per‑country obligations: US WARN/mini‑WARN thresholds and 60‑day timelines; EU collective redundancies Directive 98/59/EC consultation/notification; UK TULRCA s.188; FR CSE consultation/notification; tailor plan.",
        status=TaskStatus.PENDING,
        estimated_duration_hours=10.0,
    ...
    severance_benefits = Task(
        ...
        description="Design severance matrix (tenure/grade), benefits continuation, outplacement/career services, and local statutory overlays.",
        status=TaskStatus.PENDING,
        estimated_duration_hours=10.0,
        estimated_cost=3500.0,
    ...
    wf.constraints.extend(
        [
            ...
            Constraint(
                ...
                metadata={"notice_elements": ["nature", "timing", "contacts"], "timeline_days": 60},
            ),
            Constraint(
