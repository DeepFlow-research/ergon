...
def create_preferences() -> PreferenceWeights:
                ...
                if indicator.lower() in str(res.content or "").lower():
                    if any(
                        (
                            ...
                            for resolution in ["managed", "addressed", "mitigated", "contained"]
                        )
                    ):
                        pressure_handled += 1
                    break
    ...
    def crisis_response_speed(workflow: Workflow) -> float:
        ...
        crisis_keywords = ("crisis", "assessment", "safety", "notification", "emergency")
        fast_completed = []
        for task in workflow.tasks.values():
            ...
            if any((k in task_desc or k in task_name for k in crisis_keywords)):
                if task.status.value == "completed" and task.completed_at and workflow.started_at:
                    hours_to_complete = _safe_hours(
                        (task.completed_at - workflow.started_at).total_seconds()
                    )
                    if hours_to_complete <= 72.0:
    ...
    def consumer_safety_prioritization(workflow: Workflow) -> float:
        ...
        safety_keywords = ("safety", "consumer", "injury", "hazard", "incident", "protection")
        ...
        return min(1.0, safety_ratio * 1.5)
    ...
    def regulatory_coordination_effectiveness(workflow: Workflow) -> float:
        ...
        coordination_keywords = ("coordinate", "synchronize", "timeline", "filing", "submission")
        reg_hits = 0
        coord_hits = 0
        total_resources = 0
        for res in workflow.resources.values():
        ...
        if total_resources == 0:
            return 0.0
        reg_coverage = min(1.0, reg_hits / max(1, total_resources * 0.3))
        coord_evidence = min(1.0, coord_hits / max(1, total_resources * 0.2))
        return (reg_coverage * coord_evidence) ** 0.5
    ...
    def crisis_timeline_adherence(workflow: Workflow) -> float:
        ...
        crisis_keywords = ("crisis", "emergency", "immediate", "urgent", "notification", "safety")
        total_est = 0.0
        total_act = 0.0
        for task in workflow.tasks.values():
    ...
    crisis_management_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Require uploaded artifacts for: consumer comms (templates, schedules, dispatch proof), media decision tree, dealer kit,\nand decision authority/escalation docs with sign-offs. Return 0 if missing. [0,8]",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    operational_execution_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Award credit only if country-level reverse-logistics plans, dealer coordination agreements, customer return SOPs,\nand disposal vendor protocols are present. Cite IDs. [0,10]",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    ...
    financial_risk_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Require filed insurance claims package, coverage analysis, litigation strategy docs, and exposure matrices before credit. [0,10]",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does ready recall execution framework exist with: comprehensive tracking methodology for all 15 markets, consumer response monitoring system designed, completion rate measurement framework established, and reporting templates prepared? Return true if recall execution framework is ready for deployment, false otherwise.",
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="safety_incident_prevention_protocols_established",
            llm_prompt="Do established safety incident prevention protocols exist with: comprehensive hazard containment procedures documented, safety monitoring framework implemented, incident response protocols operational, and prevention measures validated? Return true if safety incident prevention protocols are fully established, false otherwise.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_notification_packages_prepared",
            llm_prompt="Do prepared regulatory notification packages exist with: NHTSA/Transport Canada/EU GPSR filing templates completed, national authority notification packages for 15 countries prepared, defect characterization fully documented, and coordinated submission timeline established? Return true if regulatory notification packages are prepared and ready for submission, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="market_reentry_strategy_documented",
            llm_prompt="Does documented market re-entry strategy exist with: comprehensive re-entry plan for all jurisdictions, enhanced quality protocols framework established, product redesign specifications completed, and approval pathway strategy defined? Return true if market re-entry strategy is fully documented and ready for execution, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="customer_confidence_metrics_restored",
            llm_prompt="Do restored customer confidence metrics exist with: >80% of pre-recall confidence levels achieved within 12 months, brand reputation recovery demonstrated through independent surveys, consumer trust indicators positive, and market positioning maintained? Return true if customer confidence metrics are restored, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="product_retrieval_logistics_operational",
            llm_prompt="Does operational product retrieval logistics exist with: reverse supply chain activated, dealer network coordination complete, customer return processing active, and affected inventory quarantined and disposed? Return true if product retrieval logistics are operational, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="root_cause_analysis_remediation",
            llm_prompt="Does root cause analysis and remediation exist with: technical failure investigation completed, design modifications developed, enhanced testing protocols implemented, and manufacturing process corrections validated? Return true if root cause analysis and remediation are complete, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="crisis_management_coordination",
            llm_prompt="Does crisis management coordination exist with: cross-functional recall team activated, executive communication protocols operational, 24/7 incident response capability active, and decision-making authority documented? Return true if crisis management coordination is effective, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="consumer_communication_campaign",
            llm_prompt="Does consumer communication campaign exist with: multi-channel safety notifications deployed (mail/electronic/dealer), customer service hotline operational, media relations strategy active, and regulatory-compliant messaging maintained? Return true if consumer communication campaign is effective, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_compliance_documentation",
            llm_prompt="Does regulatory compliance documentation exist with: recall effectiveness monitoring active, consumer response tracking operational, regulatory status reporting current, and audit trail maintained across all markets? Return true if regulatory compliance documentation is complete, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="financial_impact_contained",
            llm_prompt="Does contained financial impact exist with: crisis management budget parameters maintained, insurance coverage maximized, litigation exposure minimized, and financial stakeholder communication active? Return true if financial impact is contained, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="supply_chain_partners_retained",
            llm_prompt="Do retained supply chain partners exist with: enhanced quality agreements executed, supplier relationship preservation active, dealer network confidence maintained, and partnership value protected? Return true if supply chain partners are retained, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="legal_liability_management",
            llm_prompt="Does legal liability management exist with: liability management across jurisdictions, insurance claims processing active, litigation strategy developed, and legal risk mitigation operational? Return true if legal liability management is effective, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="enhanced_quality_assurance",
            llm_prompt="Does enhanced quality assurance exist with: quality protocols upgraded, testing procedures enhanced, supplier quality improvements implemented, and validation evidence documented? Return true if enhanced quality assurance is operational, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="social_media_crisis_management",
            llm_prompt="Does social media crisis management exist with: social media monitoring active, crisis response protocols implemented, reputation management strategies deployed, and online sentiment tracking operational? Return true if social media crisis management is effective, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="competitive_repositioning_plan",
            llm_prompt="Does competitive repositioning plan exist with: market positioning strategy updated, competitive advantages redefined, value proposition enhanced, and market share protection strategies active? Return true if competitive repositioning plan is established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="disposal_recycling_protocols",
            llm_prompt="Do disposal and recycling protocols exist with: environmental compliance maintained, product disposal procedures documented, recycling partnerships established, and waste management optimized? Return true if disposal and recycling protocols are operational, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="recall_effectiveness_monitoring",
            llm_prompt="Does recall effectiveness monitoring exist with: monitoring systems operational, completion rate tracking active, consumer response analysis current, and effectiveness metrics reported? Return true if recall effectiveness monitoring is operational, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
