        ...
        if any(
            ...
            if any(
                (
                    ...
                    for keyword in ["approved", "validated", "reviewed", "signed-off", "certified"]
                )
            ):
...
playbook_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate playbook adherence and deviation discipline: Are deviations logged with rationale and routed to the right approvers? Did negotiators stay within discretion boundaries and use fallbacks appropriately? Is Round 2 focused on true deltas rather than re-litigating baselines? Return a numeric score [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
handoff_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate handoff quality: Were fully executed documents ingested into CLM with key metadata (renewal dates, notice periods, liability caps, indemnities)? Was an obligations/SLA matrix published to GTM/Success with alerts configured? Return a numeric score [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does complete deal tiering and risk assessment exist with: risk tier assigned (L/M/H), target cycle time defined, required artifacts checklist established, and SLA mappings confirmed? Return true if deal tiering and risk assessment are complete, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="msa_sow_execution_ready",
            llm_prompt="Does execution-ready MSA/SOW exist with: negotiated terms finalized, redline deviations controlled and approved, legal review completed, and signature-ready documents prepared? Return true if MSA/SOW are execution-ready, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="dpa_security_review_approved",
            llm_prompt="Does approved DPA and security review exist with: data processing addendum finalized, security requirements validated, privacy compliance confirmed, and InfoSec approval secured? Return true if DPA and security review are approved, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="signature_execution_complete",
            llm_prompt="Does complete signature execution exist with: electronic signatures secured, authorization confirmations received, execution logistics completed, and contract binding confirmed? Return true if signature execution is complete, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="clm_ingest_obligation_handoff",
            llm_prompt="Does CLM ingest and obligation handoff exist with: contract management system integration complete, obligations tracked and assigned, handoff procedures executed, and contract lifecycle management active? Return true if CLM ingest and obligation handoff are operational, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="customer_data_profile_mapped",
            llm_prompt="Does mapped customer data profile exist with: data categories identified (PII/PHI/PCI), residency requirements documented, transfer protocols established, and sub-processor mapping complete? Return true if customer data profile is mapped, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="playbook_selection_optimization",
            llm_prompt="Does playbook selection optimization exist with: appropriate playbooks selected for deal type, deviation control mechanisms active, negotiation efficiency maximized, and process standardization maintained? Return true if playbook selection is optimized, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="approval_workflow_streamlined",
            llm_prompt="Does streamlined approval workflow exist with: approver hierarchy established, escalation paths defined, approval cycle time optimized, and bottleneck resolution active? Return true if approval workflow is streamlined, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="metrics_driven_cycle_time",
            llm_prompt="Does metrics-driven cycle time optimization exist with: cycle time tracking active, performance metrics monitored, optimization targets achieved, and efficiency improvements documented? Return true if metrics-driven cycle time optimization is active, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="export_control_compliance",
            llm_prompt="Does export control compliance exist with: export restrictions assessed, compliance requirements validated, documentation prepared, and regulatory adherence confirmed? Return true if export control compliance is established, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="insurance_verification_complete",
            llm_prompt="Does complete insurance verification exist with: insurance requirements validated, coverage adequacy confirmed, policy documentation secured, and risk protection established? Return true if insurance verification is complete, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="financial_terms_validation",
            llm_prompt="Does financial terms validation exist with: pricing terms confirmed, payment structures validated, revenue recognition approved, and financial compliance ensured? Return true if financial terms validation is complete, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cross_border_data_transfers",
            llm_prompt="Do cross-border data transfers exist with: transfer mechanisms documented, adequacy decisions confirmed, standard contractual clauses implemented, and data flow mapping complete? Return true if cross-border data transfers are properly managed, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="template_deviation_control",
            llm_prompt="Does template deviation control exist with: deviation tracking active, approval requirements enforced, risk assessment for deviations completed, and template integrity maintained? Return true if template deviation control is effective, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="repeatable_factory_process",
            llm_prompt="Does repeatable factory process exist with: standardized workflows documented, process repeatability ensured, quality consistency maintained, and scalability demonstrated? Return true if repeatable factory process is established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="stakeholder_coordination_active",
            llm_prompt="Does active stakeholder coordination exist with: internal alignment maintained, customer communication effective, legal coordination seamless, and cross-functional collaboration optimized? Return true if stakeholder coordination is active, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="risk_exposure_minimized",
            llm_prompt="Does minimized risk exposure exist with: legal risks assessed and mitigated, commercial risks controlled, operational risks managed, and overall risk profile optimized? Return true if risk exposure is minimized, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="contract_portfolio_optimization",
            llm_prompt="Does contract portfolio optimization exist with: contract terms standardized, portfolio risk balanced, commercial value maximized, and strategic alignment maintained? Return true if contract portfolio optimization is achieved, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
