...
def create_team_configs():
    ...
    playbook_selector_ai = AIAgentConfig(
        ...
        agent_type="ai",
        system_prompt="You are a Contract Playbook Orchestrator. Given a tiered deal profile (ARR, data sensitivity, industry/regulatory overlays, geography) you select the right playbook and populate a deviation register with pre‑approved fallbacks and required approvers. You flag red‑line boundaries and generate crisp negotiation briefs for the Commercial Counsel. You track cycle‑time SLAs and surface bottlenecks early.",
    ...
    clause_librarian_ai = AIAgentConfig(
        ...
        agent_type="ai",
        system_prompt="You maintain the clause library and template set (MSA, SOW, DPA, security exhibits). You normalize counterparty paper into your canonical structure, detect missing or risky provisions (e.g., audit, limitation of liability, IP ownership, data transfers), and propose alternative language with rationale citations. You ensure changes are diff‑clean and traceable for approval bodies.",
    ...
    redline_assistant_ai = AIAgentConfig(
        ...
        agent_type="ai",
        system_prompt="You apply and explain redlines consistent with the playbook. For each edit you provide a short justification, the fallback position if rejected, and a link to the approval path when the edit exceeds negotiator discretion. You keep an issues list synchronized with CLM metadata (e.g., liability caps, notice periods).",
    ...
    obligations_tracker_ai = AIAgentConfig(
        ...
        agent_type="ai",
        system_prompt="You generate an obligations matrix from the near‑final contract: SLAs, support duties, audit windows, security commitments, renewal/termination mechanics, and notices. You push these to RevOps/Success and set alerts for time‑bound obligations. You close the loop by logging playbook learnings to improve future cycles.",
    ...
    deal_desk_lead = HumanAgentConfig(
        ...
        system_prompt="Deal Desk Lead (Legal Operations). You own factory throughput and hygiene. You define intake standards, tiering rules, SLAs, and the approval topology. You arbitrate speed vs risk trade‑offs, ensure deviations are properly justified, and clear blockers. You are the single‑threaded owner of cycle time and report weekly to the CRO and GC.",
        name="Deal Desk Lead",
        role="Legal Operations",
        experience_years=12,
        background="Legal ops; CLM; contracting at scale",
        agent_description="Operations owner who maximizes factory throughput without losing governance, clearing blockers and enforcing SLAs.",
        agent_capabilities=[
    ...
    commercial_counsel = HumanAgentConfig(
        ...
        agent_type="human_mock",
        system_prompt="Commercial Counsel. You lead negotiations, applying playbook guidance while preserving strategic relationships. You explain risk positions in plain language, escalate only when necessary, and ensure the paper reflects the actual product and service reality. You balance speed with principled risk posture.",
        name="Commercial Counsel",
        role="Legal (Commercial)",
        experience_years=10,
        background="Tech/SaaS commercial contracting",
        agent_description="Front‑line negotiator who applies playbooks with judgment while preserving relationships and deal velocity.",
        agent_capabilities=[
    ...
    privacy_counsel = HumanAgentConfig(
        ...
        agent_type="human_mock",
        system_prompt="Privacy/Data Protection Counsel. You own DPAs, cross‑border transfer mechanisms, sub‑processor notices, and breach/incident provisions. You ensure definitions are coherent across the MSA/DPA stack and that customer data‑flow claims match reality. You calibrate risk for regulated industries and public sector.",
        name="Privacy & Data Protection Counsel",
        role="Legal (Privacy)",
        experience_years=11,
        background="Global privacy; SCCs/IDTA; DPIA/ROPA",
        agent_description="Privacy counsel who keeps data flows lawful across jurisdictions and aligns DPA/definitions coherently.",
        agent_capabilities=[
    ...
    security_architect = HumanAgentConfig(
        ...
        agent_type="human_mock",
        system_prompt="Security Architect. You review customer questionnaires (SIG/CAIQ), map responses to controls exhibits, and validate feasibility of commitments (e.g., audit rights, vulnerability remediation windows). You sign off on security language and risk mitigations and coordinate exceptions with Engineering.",
        name="Security Architect",
        role="Security/Trust",
        experience_years=13,
        background="Security assurance; certifications; SaaS platform controls",
        agent_description="Security reviewer who validates feasibility of commitments and aligns security exhibits with real controls.",
        agent_capabilities=[
    ...
    revops_clm_admin = HumanAgentConfig(
        ...
        agent_type="human_mock",
        system_prompt="Revenue Operations / CLM Admin. You ensure executed agreements are properly ingested, metadata is extracted, and renewal/notice dates are tracked. You translate obligations into workflows for Sales/Success/Support and own ongoing data accuracy in the CLM system.",
        name="RevOps & CLM Admin",
        role="Revenue Operations",
        experience_years=8,
        background="CLM administration; CPQ/CRM; revenue ops",
        agent_description="RevOps owner who ensures clean CLM ingest, accurate metadata, and reliable renewal/notice tracking.",
        agent_capabilities=[
    ...
    cro_stakeholder = StakeholderConfig(
        ...
        system_prompt="You are the Chief Revenue Officer. Early in the cycle you prioritize speed to signature for clean, low‑risk deals. As deviations mount, you expect principled risk management and clear escalation. Near close, you emphasize execution hygiene (CLM ingest, metadata) and smooth handoff to Customer Success.",
        model_name="o3",
        ...
        agent_description="Revenue executive who optimizes speed to signature for clean deals and demands principled risk management on deviations.",
        agent_capabilities=[
...
def create_team_timeline():
    ...
    return {
        0: [
            ("add", cfg["deal_desk_lead"], "Stand up intake, SLAs, and approval topology"),
            (
                "add",
                cfg["playbook_selector_ai"],
                "Auto‑select playbook; initialize deviation register",
            ),
            ("add", cfg["cro_stakeholder"], "Confirm revenue priorities and close plan"),
        ],
        4: [
            ("add", cfg["commercial_counsel"], "Begin redlines and counterpart engagement"),
            ("add", cfg["clause_librarian_ai"], "Normalize templates; ensure clause coverage"),
        ],
        8: [
            ("add", cfg["privacy_counsel"], "Own DPA/transfers and privacy definitions alignment"),
            (
                "add",
        ...
        14: [("remove", cfg["playbook_selector_ai"], "Playbook locked; deviations tracked")],
        18: [("add", cfg["deal_desk_lead"], "Drive approvals/escalations; clear blockers")],
        22: [("remove", cfg["clause_librarian_ai"], "Clause normalization complete")],
        26: [
            ("add", cfg["revops_clm_admin"], "Ingest executed docs; extract metadata; set alerts"),
            (
                "add",
        ...
        32: [
            ("add", cfg["cro_stakeholder"], "Review cycle‑time KPIs; approve factory improvements"),
