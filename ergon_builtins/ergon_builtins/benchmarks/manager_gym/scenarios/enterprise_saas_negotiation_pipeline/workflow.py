...
def create_workflow() -> Workflow:
    wf = Workflow(
        ...
        workflow_goal="Stand up a repeatable, metrics‑driven commercial contracting factory for Enterprise SaaS deals. Objectives: qualify and tier deals, select playbooks, run redlines with controlled deviations, complete DPA/security reviews, secure approvals, and execute signatures with CLM ingest and obligation handoff. Optimize cycle time without increasing risk exposure.",
        owner_id=uuid4(),
    )
    intake_qualification = Task(
        name="Intake & Risk Tiering",
        description="Deal intake form; risk tiering based on ARR, data sensitivity, residency, industry/regime (e.g., finance/health), and requested deviations. Output: tier, target cycle time, and required artifacts checklist.",
    ...
    customer_profile = Task(
        ...
        description="Map data categories (PII/PHI/PCI), residency, transfers, processors/sub‑processors, and product usage to inform DPA and security scopes.",
        status=TaskStatus.PENDING,
    ...
    package_assembly = Task(
        ...
        description="Assemble initial contracting package: company paper MSA + SOW template, DPA, InfoSec/controls exhibits, SLAs, support/maintenance, pricing order form.",
        status=TaskStatus.PENDING,
        estimated_duration_hours=6.0,
        estimated_cost=1200.0,
    ...
    playbook_selection = Task(
        ...
        description="Select applicable playbook based on tier; pre‑approve fallback positions and define boundaries for negotiator discretion. Log anticipated deviations and approval path.",
        status=TaskStatus.PENDING,
    ...
    wf.constraints.extend(
        [
            Constraint(
                ...
                metadata={"approvers": ["legal_exec", "security", "finance", "product"]},
            ),
            Constraint(
                ...
                enforcement_level=0.95,
                applicable_task_types=["Contract Package Assembly", "Redlines Round 1 (Legal)"],
                metadata={"mechanisms": ["SCCs", "UK IDTA", "Addenda/Appropriate Safeguards"]},
            ),
            Constraint(
            ...
            Constraint(
                ...
                applicable_task_types=["CLM Ingest & Metadata", "Obligations & SLA Handoff"],
                metadata={
                    "required_metadata": [
                        "renewal_date",
                        "notice_period",
