...
def create_preferences() -> PreferenceWeights:
                ...
                if indicator.lower() in str(res.content or "").lower():
                    if any(
                        (
                            ...
                            for resolution in ["addressed", "resolved", "improved", "corrected"]
                        )
                    ):
                        pressure_handled += 1
                    break
    ...
    def academic_quality_artifacts(workflow: Workflow) -> float:
        ...
        if not completed:
            return 0.0
        quality_keywords = ("external examiner", "kpi", "standards", "assessment", "quality")
        quality_outputs = 0
        total_outputs = 0
        for t in completed:
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does submitted compliant OfS evidence pack exist with: evidence pack submitted on time, no material deficiencies identified, ≤2 rounds of regulator clarifications, and OfS conditions A-E addressed comprehensively? Return: 20.0 if all conditions are met, removing 2 for every condition not met to a minimum of 0.0.",
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="quality_standards_compliance_b_conditions",
            llm_prompt="Does quality standards compliance with B-conditions exist with: course clusters mapped to B1-B6 conditions, external examiner summaries provided, continuation/completion/progression KPIs met, and remediation actions documented? Return 18.0 if all conditions are met, removing 2 for every condition not met to a minimum of 0.0. Return 5.0 if there is evidence of any of these being planned but not yet completed.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cma_consumer_law_compliance_audit",
            llm_prompt="Does CMA consumer law compliance audit exist with: CMA-compliant prospectus and contracts confirmed, clear/fair/transparent information verified, legal review completed, and no unresolved unfair terms findings? Return 15.0 if all conditions are met, removing 2 for every condition not met to a minimum of 0.0. Return 5.0 if there is evidence of any of these being planned but not yet completed.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="app_accepted_ofs_valid_credible",
            llm_prompt="Does OfS-accepted APP exist with: Access and Participation Plan updated, monitoring data provided, gap analysis completed, OfS acceptance as valid and credible confirmed? Return true if APP is OfS-accepted as valid and credible, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="ukvi_sponsorship_compliance_maintained",
            llm_prompt="Does maintained UKVI sponsorship compliance exist with: attendance monitoring records current, CAS issuance logs documented, reporting evidence provided, and sponsor license maintained without major findings? Return true if UKVI sponsorship compliance is maintained, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="prevent_duty_documentation_compliant",
            llm_prompt="Does compliant Prevent duty documentation exist with: risk assessment completed, training coverage documented, incident logs maintained, and governing-body oversight consistent with OfS monitoring? Return true if Prevent duty documentation is compliant, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="governance_package_decisions_documented",
            llm_prompt="Does documented governance package with decisions exist with: Council/Senate/Quality Committee minutes complete, oversight decisions documented, sign-offs secured, and risk registers/escalation logs maintained? Return true if governance package with decisions is documented, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="data_quality_assurance_hesa_validated",
            llm_prompt="Does HESA-validated data quality assurance exist with: HESA Data Futures submissions validated, internal audit checks completed, data integrity statements signed, and data quality confirmed? Return true if data quality assurance is HESA-validated, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="student_outcomes_tef_aligned",
            llm_prompt="Does TEF-aligned student outcomes narrative exist with: Teaching Excellence Framework indicators addressed, continuous improvement evidence provided, student outcomes narrative compelling, and TEF alignment demonstrated? Return true if student outcomes are TEF-aligned, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="financial_sustainability_evidence",
            llm_prompt="Does financial sustainability evidence exist with: financial sustainability demonstrated, financial projections validated, sustainability metrics met, and financial governance robust? Return true if financial sustainability evidence is compelling, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="student_protection_measures_documented",
            llm_prompt="Do documented student protection measures exist with: student protection plan current, protection measures operational, student interests safeguarded, and protection compliance demonstrated? Return true if student protection measures are documented, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="external_examiner_summaries_validated",
            llm_prompt="Do validated external examiner summaries exist with: external examiner reports comprehensive, quality assurance validation completed, academic standards confirmed, and examiner feedback addressed? Return true if external examiner summaries are validated, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="equality_opportunity_governance_oversight",
            llm_prompt="Does equality opportunity governance oversight exist with: equality of opportunity monitored, governance oversight demonstrated, gap analysis completed, and diversity metrics tracked? Return true if equality opportunity governance oversight is effective, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="academic_standards_baseline_compliance",
            llm_prompt="Does academic standards baseline compliance exist with: academic standards maintained, baseline requirements met, quality thresholds achieved, and standards governance operational? Return true if academic standards baseline compliance is demonstrated, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="course_information_transparency",
            llm_prompt="Does course information transparency exist with: course information clear and accurate, costs and contact hours transparent, student information accessible, and transparency standards met? Return true if course information transparency is achieved, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_correspondence_tracking",
            llm_prompt="Does regulatory correspondence tracking exist with: OfS communications logged, regulator interactions documented, compliance correspondence maintained, and response tracking active? Return true if regulatory correspondence tracking is comprehensive, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="continuous_improvement_initiatives",
            llm_prompt="Do continuous improvement initiatives exist with: improvement initiatives identified and implemented, enhancement activities documented, quality development ongoing, and improvement culture demonstrated? Return true if continuous improvement initiatives are active, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="sector_benchmarking_alignment",
            llm_prompt="Does sector benchmarking alignment exist with: sector benchmarks analyzed, performance compared to peers, best practices identified, and sector alignment demonstrated? Return true if sector benchmarking alignment is established, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
