...
def create_tech_acquisition_integration_preferences() -> PreferenceWeights:
    ...
    def integration_timeline_adherence(workflow: Workflow) -> float:
        ...
        for task in workflow.tasks.values():
            if task.completed_at is not None:
                ...
                if any(
                    ...
                    if full_integration_time is None or task.completed_at < full_integration_time:
        ...
        if systems_integration_time is not None:
            sys_days = (
                _safe_hours((systems_integration_time - workflow.started_at).total_seconds()) / 24.0
            )
            if sys_days <= 90.0:
                score += 0.5
            elif sys_days <= 120.0:
        ...
        if full_integration_time is not None:
            full_days = (
                _safe_hours((full_integration_time - workflow.started_at).total_seconds()) / 24.0
            )
            if full_days <= 180.0:
                score += 0.5
    ...
    def regulatory_approval_progress(workflow: Workflow) -> float:
        ...
        approval_score = min(1.0, approval_signals / 3.0)  # Expect at least 3 approval milestones
