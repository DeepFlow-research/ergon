...
def create_banking_license_preferences() -> PreferenceWeights:
    ...
    def regulatory_documentation_completeness(workflow: Workflow) -> float:
        ...
        keywords = ("application", "compliance", "regulatory", "policy", "procedure", "framework")
        total = 0
        hits = 0
        for res in workflow.resources.values():
            total += 1
    ...
    def operational_readiness_density(workflow: Workflow) -> float:
        ...
        infra_keywords = ("infrastructure", "system", "operational", "setup", "implementation")
    ...
    def regulatory_milestone_adherence(workflow: Workflow) -> float:
        ...
        regulatory_keywords = ("application", "approval", "regulatory", "submission", "review")
        reg_tasks = []
    ...
    def compliance_framework_coverage(workflow: Workflow) -> float:
        ...
        compliance_areas = ("aml", "bsa", "kyc", "cip", "sanctions", "monitoring", "reporting")
        coverage_count = 0
        for res in workflow.resources.values():
            try:
                content = (res.content or "").lower()
    ...
    stakeholder_coordination_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Require evidence of regulator liaison protocols, project governance artifacts (RACI, escalation),\nand legal counsel coordination records. Award credit only with uploaded documents/approvals. [0,8]",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does OCC (Office of the Comptroller of the Currency) license approval exist with: final approval letter, all regulatory conditions satisfied, federal branch authorization, and operational commencement approval? Return true if OCC license is officially granted and operational, false otherwise.",
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="federal_reserve_non_objection",
            llm_prompt="Does Federal Reserve non-objection exist with: formal non-objection letter, supervision arrangements confirmed, consolidated supervision agreement, and compliance attestation? Return true if Federal Reserve non-objection is secured, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="home_country_regulator_approval",
            llm_prompt="Does home country regulator (ECB/national authority) approval exist with: US market entry authorization, consolidated supervision attestation, capital adequacy confirmation, and cross-border oversight agreement? Return true if home country approval is secured, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="aml_bsa_program_implemented",
            llm_prompt="Does implemented AML/BSA compliance program exist with: written compliance policies, customer identification procedures, enhanced due diligence protocols, independent testing, and BSA officer appointment? Return true if AML/BSA program is fully implemented and validated, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="capital_structure_established",
            llm_prompt="Does established capital structure exist with: $50M minimum capital deposited, CED (Capital Equivalent Deposit) arrangements confirmed, liquidity facilities secured, and ongoing funding sources documented? Return true if capital requirements are fully satisfied, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="risk_management_framework_operational",
            llm_prompt="Does operational risk management framework exist with: credit risk policies, market risk controls, liquidity management procedures, operational risk assessment, and regulatory reporting systems? Return true if risk management framework is operational, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="operational_infrastructure_deployed",
            llm_prompt="Does deployed operational infrastructure exist with: IT systems operational, physical office setup complete, correspondent banking relationships established, and service delivery capabilities tested? Return true if operational infrastructure is fully deployed, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="senior_management_appointed",
            llm_prompt="Does appointed senior management team exist with: all key positions filled, background checks completed, regulatory fitness confirmed, and board governance established? Return true if senior management is appointed and qualified, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="business_plan_approved",
            llm_prompt="Does approved business plan exist with: comprehensive market analysis, target customer segments defined, product offerings documented, 3-year financial projections, and regulatory acceptance confirmed? Return 8.0 if there is evidence of a business plan being approved and comprehensive, 4.0 if there is evidence of a business plan being drafted or planned but it is not complete, 0.0 otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="corporate_governance_established",
            llm_prompt="Does established corporate governance exist with: board structure implemented, committee charters approved, governance policies documented, and decision-making authorities defined? Return 8.0 if there is evidence of corporate governance being established, 4.0 if there is evidence of corporate governance being established but it is not complete, 0.0 otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_reporting_systems",
            llm_prompt="Do regulatory reporting systems exist with: automated reporting capabilities, data quality controls, regulatory submission procedures, and compliance monitoring tools? Return 6.0 if there is evidence of specifications for regulatory reporting systems are drafted, 3.0 if there is evidence of regulatory reporting systems being established but it is not complete, 0.0 otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="due_diligence_completed",
            llm_prompt="Does completed due diligence exist with: beneficial ownership verification, management background checks completed, corporate affiliations reviewed, and regulatory validation secured? Return true if comprehensive due diligence is completed, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="legal_structure_finalized",
            llm_prompt="Does finalized legal structure exist with: US entity incorporation complete, legal documentation package executed, regulatory structure confirmed, and operational agreements signed? Return true if legal structure is finalized, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="market_entry_strategy_documented",
            llm_prompt="Does documented market entry strategy exist with: competitive analysis completed, customer acquisition strategy defined, revenue projections validated, and go-to-market timeline established? Return true if market entry strategy is documented, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_conditions_satisfied",
            llm_prompt="Is there evidence that any regulatory conditions relevent to banking operations have been broken? Return 4.0 if there is no evidence of a regulatory condition being broken, 0.0 otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="fdic_insurance_secured",
            llm_prompt="Does FDIC insurance exist with: application submitted, coverage approved, premium payments arranged, and deposit insurance operational (if applicable)? Return true if FDIC insurance is secured or N/A, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
