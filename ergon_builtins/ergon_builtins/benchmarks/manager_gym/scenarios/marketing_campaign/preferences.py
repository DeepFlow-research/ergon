...
def _require_external_validation(workflow: Workflow, validation_keywords: List[str]) -> float:
    ...
    for task in workflow.tasks.values():
        if any(
            (keyword.lower() in (task.description or "").lower() for keyword in validation_keywords)
        ):
            if any(
                (
                    keyword.lower() in str(res.content or "").lower()
                    for res in workflow.resources.values()
                    for keyword in ["approved", "validated", "reviewed", "certified", "audited"]
                )
            ):
...
def rule_foundations_ready(workflow: Workflow) -> float:
    ...
    """
    return 0.5 * _task_completed(workflow, "Web & Conversion Experience") + 0.5 * _task_completed(
        workflow, "CRM Enablement"
    )
...
def rule_experimentation_live(workflow: Workflow) -> float:
    ...
    """
    return 0.5 * _task_completed(workflow, "Measurement Framework") + 0.5 * _task_completed(
        workflow, "Experimentation & Optimization"
    )
...
performance_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate whether the campaign is set up to generate qualified pipeline efficiently: (a) clear ICPs/personas and segment-specific offers; (b) working lead capture with consent, scoring, dedup, and routing SLAs; (c) defined KPIs with UTM governance; and (d) experimentation plan that can impact CAC and conversion rates. Cite concrete workflow evidence (tasks, artifacts, dashboards). Return a numeric score [0, 10].",
        max_score=10.0,
...
brand_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Assess whether creative assets reflect a coherent brand system: consistent narrative across hero/variants, inclusive language, and alignment to the messaging house. Consider QA (alt text/captions) and partner co-branding guidance. Return a numeric score [0, 8].",
        max_score=8.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
        name="brand_lift_readiness",
        llm_prompt="Assess readiness to measure brand lift: sampling plan, survey constructs (ad recall, awareness, consideration), fielding schedule, and integration with dashboards. Return a numeric score [0, 6].",
        max_score=6.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
compliance_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate environmental claims for compliance and substantiation: avoid broad unqualified 'green' claims; require specific, evidenced claims with clear qualifications; maintain a substantiation log (e.g., RECs, LCA references). Return a numeric score [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
        name="privacy_and_email_compliance",
        llm_prompt="Assess consent and email compliance posture: explicit consent for non-essential cookies and direct marketing; functional one‑click unsubscribe; clear sender identity and postal address. Return a numeric score [0, 8].",
        max_score=8.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
accessibility_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate whether key campaign web pages and assets conform to WCAG 2.1 AA: text alternatives, captions, sufficient contrast, focus order, and keyboard access; include evidence of automated and manual checks. Return a numeric score [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does operational brand tracking framework exist with: measurement approach documented, key metrics defined, tracking methodology outlined, and reporting framework established? Return true if brand tracking components are documented and ready for deployment, false otherwise.",
            max_score=18.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="lead_generation_system_designed",
            llm_prompt="Does designed lead generation system exist with: lead scoring strategy documented, tracking frameworks established, target audience segments defined, and measurement approach specified? Return true if lead generation components are documented and ready for implementation, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="seo_optimization_strategy_implemented",
            llm_prompt="Does implemented SEO optimization strategy exist with: keyword research and strategy documented, content optimization guidelines established, SEO best practices framework created, and tracking approach defined? Return true if SEO optimization components are documented and ready for execution, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="b2b_sales_pipeline_framework_ready",
            llm_prompt="Does ready B2B sales pipeline framework exist with: opportunity qualification approach documented, scoring criteria established, handoff procedures defined, and pipeline tracking methodology created? Return true if B2B pipeline components are documented and ready for implementation, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="messaging_claims_substantiation",
            llm_prompt="Does messaging claims substantiation exist with: substantiation log maintained, environmental claims validated, no greenwashing violations, and compliance audit passed? Return true if messaging claims substantiation is complete and compliant, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="wcag_accessibility_compliance",
            llm_prompt="Does WCAG accessibility compliance exist with: WCAG 2.1 AA compliance achieved for key web assets, accessibility audit completed, user testing with assistive technologies performed, and compliance certified? Return true if WCAG accessibility compliance is achieved, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="consent_opt_in_rates_achieved",
            llm_prompt="Do achieved consent opt-in rates exist with: ≥85% consent opt-in rates maintained, privacy compliance validated, consent management system operational, and GDPR/CCPA compliance confirmed? Return true if consent opt-in rates are achieved, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="integrated_campaign_execution",
            llm_prompt="Does integrated campaign execution exist with: multi-channel strategy deployed (paid/owned/earned), cross-channel messaging consistency maintained, campaign coordination operational, and unified customer experience delivered? Return true if integrated campaign execution is successful, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="measurement_framework_operational",
            llm_prompt="Does operational measurement framework exist with: MMM/MTA-compatible tracking implemented, UTM governance established, attribution models validated, and measurement accuracy confirmed? Return true if measurement framework is operational, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="creative_toolkit_deployed",
            llm_prompt="Does deployed creative toolkit exist with: brand guidelines implemented, creative assets produced across channels, visual identity consistency maintained, and creative performance tracked? Return true if creative toolkit is deployed and effective, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="pr_analyst_program_active",
            llm_prompt="Does active PR and analyst program exist with: media relations strategy executed, analyst engagement program operational, thought leadership content published, and industry recognition achieved? Return true if PR and analyst program is active, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="events_field_marketing_executed",
            llm_prompt="Does executed events and field marketing exist with: events program launched, field marketing campaigns deployed, in-person engagement activities completed, and lead generation from events tracked? Return true if events and field marketing are executed, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="partner_co_marketing_assets",
            llm_prompt="Do partner co-marketing assets exist with: partner marketing materials developed, co-marketing agreements executed, joint campaigns launched, and partner channel performance tracked? Return true if partner co-marketing assets are operational, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="experimentation_cadence_established",
            llm_prompt="Does established experimentation cadence exist with: A/B testing framework operational, test planning and execution systematic, results analysis automated, and optimization cycles active? Return true if experimentation cadence is established, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="executive_dashboard_operational",
            llm_prompt="Does operational executive dashboard exist with: weekly performance reporting active, KPI tracking automated, executive stakeholder access configured, and actionable insights provided? Return true if executive dashboard is operational, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="crm_lead_routing_system",
            llm_prompt="Does CRM lead routing system exist with: automated lead scoring implemented, routing workflows configured, sales team integration complete, and lead management processes operational? Return true if CRM lead routing system is functional, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="web_conversion_flows_optimized",
            llm_prompt="Do optimized web conversion flows exist with: conversion funnel analysis completed, user experience optimization implemented, conversion rate improvements achieved, and customer journey mapped? Return true if web conversion flows are optimized, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="audience_persona_validation",
            llm_prompt="Does audience persona validation exist with: target personas researched and validated, customer interviews completed, persona-based messaging tested, and audience segmentation optimized? Return true if audience persona validation is complete, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
