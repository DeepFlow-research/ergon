...
def create_workflow() -> Workflow:
    wf = Workflow(
        ...
        workflow_goal="Coordinate legal‑privileged, cross‑functional response to a suspected global data breach impacting EU/US customers and partners. Objectives: preserve evidence, scope and contain the incident, determine notification obligations and timing, execute regulator/customer/partner communications, and implement remediation. Produce a defensible post‑incident report and board briefing with corrective action plan.",
        owner_id=uuid4(),
    )
    detection_triage = Task(
        name="Detection & Legal Triage",
        description="Open incident under legal privilege; classify severity; appoint Incident Commander (Legal). Ensure initial facts are captured and clock start is noted for notification SLAs.",
    ...
    evidence_preservation = Task(
        ...
        description="Snapshot affected systems; collect logs/artifacts; enforce chain‑of‑custody and access controls.",
        status=TaskStatus.PENDING,
    ...
    forensics_scoping = Task(
        ...
        description="Establish attack vector hypotheses; identify impacted systems/accounts; reconstruct a timeline of events.",
        status=TaskStatus.PENDING,
    ...
    data_mapping = Task(
        ...
        description="Classify data (PII/PHI/PCI/sensitive categories), link to jurisdictions and storage/processing locations, and estimate affected data subjects/records.",
        status=TaskStatus.PENDING,
    ...
    obligations_matrix = Task(
        ...
        description="Determine whether/when to notify regulators, individuals, and partners across jurisdictions; capture legal bases, timelines, and content requirements.",
        status=TaskStatus.PENDING,
    ...
    comms_plan = Task(
        ...
        description="Draft regulator notifications, customer/partner notices, FAQs, employee comms, and press holding statements; define spokespersons and Q&A protocols.",
        status=TaskStatus.PENDING,
        estimated_duration_hours=12.0,
