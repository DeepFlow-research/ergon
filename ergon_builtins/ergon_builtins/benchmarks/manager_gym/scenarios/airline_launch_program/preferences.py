...
def create_preferences() -> PreferenceWeights:
    ...
    def airport_coordination_effectiveness(workflow: Workflow) -> float:
        ...
        airport_keywords = ("slot", "airport", "handling", "ground operations", "coordination")
    ...
    def caa_engagement_tracking(workflow: Workflow) -> float:
        ...
        caa_keywords = ("caa", "inspector", "finding", "approval", "submission", "response")
    ...
    speed_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Evaluate adherence to certification timeline including AOC and Operating Licence application processing,\n                proving flight scheduling, and critical path management. Cite evidence.\n                Output numeric score [0, 7].",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="regulatory_response_timeliness",
            llm_prompt="Assess timeliness of regulatory responses including CAA finding closure,\n                application submissions, and inspection support. Output numeric score [0, 5].",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does ready AOC application package exist with: complete CAA application forms prepared, Operations Manual documentation compiled, Supporting evidence assembled, and submission checklist validated? Return true if AOC application package is ready for submission, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="operating_licence_application_prepared",
            llm_prompt="Does prepared Operating Licence application exist with: comprehensive application documentation ready, financial fitness demonstration compiled, UK ownership/control validation documented, and insurance framework established? Return true if Operating Licence application is prepared for submission, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="airworthiness_certification_framework_ready",
            llm_prompt="Does ready airworthiness certification framework exist with: CofA application documentation prepared, ARC renewal procedures established, aircraft registration framework documented, and maintenance program designed? Return true if airworthiness certification framework is ready for implementation, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="aviation_security_programme_documented",
            llm_prompt="Does documented aviation security programme exist with: NASP compliance framework established, security programme documentation prepared, staff vetting procedures defined, and security training curriculum designed? Return true if security programme is fully documented and ready for implementation, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="operations_manual_comprehensive",
            llm_prompt="Does comprehensive Operations Manual exist with: OM-A General Procedures documented, OM-B Aircraft Operating Procedures completed, OM-C Route & Aerodrome Information compiled, and OM-D Training Program designed? Return true if complete Operations Manual is documented and ready for CAA review, false otherwise.",
            max_score=9.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="safety_management_system_operational",
            llm_prompt="Does operational Safety Management System exist with: safety policy implementation, risk management processes, safety assurance monitoring, and safety promotion programs? Return true if SMS is fully operational per ICAO Annex 19, false otherwise.",
            max_score=9.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="camo_part145_arrangements_validated",
            llm_prompt="Do validated airworthiness arrangements exist with: Part-CAMO approval or contracts, Part-145 maintenance organization arrangements, reliability monitoring systems, and MEL/GMEL approvals? Return true if continuing airworthiness arrangements are validated, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="insurance_coverage_binding",
            llm_prompt="Do binding insurance certificates exist with: passenger liability coverage, baggage and cargo liability, third-party liability coverage, and regulatory minimum compliance? Return true if all required insurance is binding and compliant, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="nominated_postholders_appointed",
            llm_prompt="Are nominated postholders appointed with: Flight Operations postholder, Ground Operations postholder, Continuing Airworthiness postholder, Crew Training postholder, Safety postholder, Security postholder, and CAA competency requirements met? Return true if all required postholders are appointed and qualified, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="proving_flights_completed",
            llm_prompt="Do completed proving flights exist with: successful flight demonstrations, operational competency validation, CAA inspector observations, and no unresolved findings? Return true if proving flights are successfully completed, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="airport_slots_confirmed",
            llm_prompt="Do confirmed airport slots exist with: slot confirmations at coordinated airports, ACL coordination evidence, handling contract agreements, and operational access arrangements? Return true if airport slots are confirmed and operational, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="staff_vetting_training_complete",
            llm_prompt="Does complete staff vetting and training exist with: 100% security vetting completion, aviation security training records, competency assessments, and ongoing training programs? Return true if all staff vetting and training is complete, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="ground_handling_contracts_finalized",
            llm_prompt="Do finalized ground handling contracts exist with: passenger handling agreements, baggage handling contracts, cargo handling arrangements, and ramp service contracts? Return true if ground handling contracts are finalized, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="maintenance_programs_approved",
            llm_prompt="Do approved maintenance programs exist with: aircraft maintenance programs, MEL (Minimum Equipment List) approval, GMEL (General Minimum Equipment List), and reliability monitoring? Return true if maintenance programs are approved, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="financial_fitness_demonstrated",
            llm_prompt="Does demonstrated financial fitness exist with: business plan documentation, cash flow projections, funding evidence, and financial monitoring systems? Return true if financial fitness is demonstrated and documented, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="uk_principal_place_business",
            llm_prompt="Does UK principal place of business exist with: UK corporate structure, principal place of business establishment, ownership/control validation, and governance framework? Return true if UK principal place of business is established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="disruption_management_procedures",
            llm_prompt="Do disruption management procedures exist with: disruption management plans, passenger rights compliance under UK261, contingency procedures, and customer communication protocols? Return true if disruption management procedures are established, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="dangerous_goods_approval",
            llm_prompt="Does dangerous goods approval exist with: CAA Form SRG2807 submission, DG training program approval, Operations Manual DG procedures, and compliance monitoring integration? Return true if dangerous goods approval is obtained (or N/A if not applicable), false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="final_governance_approvals",
            llm_prompt="Do final governance approvals exist with: Board of Directors approval, Accountable Manager certification, postholder sign-offs, and launch readiness validation? Return true if final governance approvals are obtained, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="caa_inspection_findings_closed",
            llm_prompt="Are CAA inspection findings closed with: inspection completion records, corrective action implementation, finding closure confirmations, and compliance demonstration? Return true if all CAA findings are closed, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
