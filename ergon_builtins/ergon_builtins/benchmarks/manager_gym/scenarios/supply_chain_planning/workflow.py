...
def create_workflow() -> Workflow:
    wf = Workflow(
        ...
        workflow_goal="Integrated S&OP for a Suez-based logistics operator covering demand forecasting, vessel/convoy planning, capacity & intermodal allocation, customs/DG compliance, and execution control with risk contingencies.",
        owner_id=uuid4(),
    )
    demand_forecast = Task(
        name="Demand & Volume Forecast",
        description="Build multi-horizon demand forecast combining customer bookings pipeline, seasonality, and disruption signals; publish volume bands with confidence intervals.",
    ...
    network_capacity = Task(
        ...
        description="Allocate vessel slots, rail/road capacity, yard space, and warehouse throughput; establish tactical plan for the planning window with buffers.",
        status=TaskStatus.PENDING,
    ...
    transit_slot_planning = Task(
        ...
        description="Coordinate canal convoy/slot planning and align with departure/arrival rotations; build alternatives in case of delays.",
        status=TaskStatus.PENDING,
    ...
    customs_docs = Task(
        ...
        description="Prepare export/import documentation (manifests, certificates) with data validation and pre-arrival filing where applicable.",
        status=TaskStatus.PENDING,
        estimated_duration_hours=10.0,
        estimated_cost=900.0,
        dependency_task_ids=[network_capacity.task_id],
    ...
    daily_ops = Task(
        ...
        dependency_task_ids=[control_tower.task_id, gate_cutoffs.task_id, dangerous_goods.task_id],
    )
    kpi_review = Task(
    ...
    wf.constraints.extend(
        [
            ...
            Constraint(
                ...
                constraint_type="hard",
                enforcement_level=1.0,
                applicable_task_types=["Customs & Documentation", "Gate-In & Cutoff Management"],
                metadata={},
            ),
            Constraint(
