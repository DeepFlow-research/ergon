        ...
        if any(
            ...
            if any(
                (
                    ...
                    for keyword in ["approved", "validated", "confirmed", "verified", "certified"]
                )
            ):
...
service_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate service reliability using available artifacts and metrics: On‑Time‑In‑Full (OTIF) targets vs actuals, dwell time at yards/ports, and schedule adherence. Assess whether the control tower triages exceptions by SLA/impact and documents resolutions. Return a numeric score [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
        name="schedule_integrity",
        llm_prompt="Assess schedule integrity: confirmed Suez transit slots/convoys, synchronized rotations/ETAs, and timely publication of cutoffs (docs, gate‑in, VGM). Return a numeric score [0, 8].",
        max_score=8.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
compliance_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate customs posture: HS code accuracy, manifest integrity, pre‑arrival/advance filings on time, and certificates/permits completeness, with audit trail. Return a numeric score [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
        name="dangerous_goods_screening_quality",
        llm_prompt="Assess Dangerous Goods (DG) screening quality: correct classification, required declarations, stowage restrictions, and load holds where applicable. Return a numeric score [0, 8].",
        max_score=8.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
cost_rubrics: List[WorkflowRubric] = [
    WorkflowRubric(
        ...
        llm_prompt="Evaluate cost efficiency: equipment/yard/warehouse utilization vs plan, cost variance to budget, and evidence of optimization actions (e.g., load consolidation, shift optimization). Return [0, 10].",
        max_score=10.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
        name="empty_repositioning_efficiency",
        llm_prompt="Assess empty repositioning efficiency: balance of empties across depots, avoided deadhead moves, and alignment to demand bands. Return a numeric score [0, 8].",
        max_score=8.0,
        run_condition=RunCondition.ON_COMPLETION,
    ),
    WorkflowRubric(
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does achieved demand forecast accuracy exist with: multi-horizon demand forecast completed, seasonality analysis integrated, confidence intervals published, and forecast accuracy validated against historical performance? Return true if demand forecast accuracy is achieved and reliable, false otherwise.",
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="vessel_capacity_allocation_optimized",
            llm_prompt="Does optimized vessel capacity allocation exist with: vessel slots allocated by lane, safety stock maintained for priority customers, capacity utilization optimized, and allocation strategy documented? Return true if vessel capacity allocation is optimized, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="customs_dg_compliance_operational",
            llm_prompt="Does operational customs and DG compliance exist with: dangerous goods procedures validated, customs documentation automated, compliance tracking active, and regulatory requirements met across all lanes? Return true if customs and DG compliance are operational, false otherwise.",
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="intermodal_network_coordination",
            llm_prompt="Does intermodal network coordination exist with: rail/road capacity balanced, yard space allocated efficiently, warehouse throughput optimized, and modal integration seamless? Return true if intermodal network coordination is effective, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="execution_control_system_active",
            llm_prompt="Does active execution control system exist with: real-time tracking operational, performance monitoring automated, exception handling procedures active, and control metrics reported? Return true if execution control system is active and effective, false otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="risk_contingency_plans_deployed",
            llm_prompt="Do deployed risk contingency plans exist with: disruption scenarios identified, contingency strategies documented, risk mitigation protocols active, and emergency response procedures tested? Return true if risk contingency plans are deployed, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="customer_booking_pipeline_integrated",
            llm_prompt="Does integrated customer booking pipeline exist with: CRM pipeline integrated, no-show/cancel rates incorporated, booking forecasts accurate, and customer demand patterns analyzed? Return true if customer booking pipeline is integrated, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="performance_kpi_dashboard_operational",
            llm_prompt="Does operational performance KPI dashboard exist with: key metrics tracked in real-time, performance indicators automated, dashboard accessibility ensured, and decision-support analytics active? Return true if performance KPI dashboard is operational, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="yard_warehouse_throughput_optimized",
            llm_prompt="Does optimized yard and warehouse throughput exist with: yard slots balanced to volumes, warehouse shifts aligned to forecasts, throughput bottlenecks identified and resolved, and efficiency maximized? Return true if yard and warehouse throughput are optimized, false otherwise.",
            max_score=8.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="disruption_weather_signals_monitoring",
            llm_prompt="Does disruption and weather signals monitoring exist with: weather indicators tracked, port congestion monitored, disruption signals integrated into forecasts, and scenario planning active? Return true if disruption and weather signals monitoring is operational, false otherwise.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="supplier_vendor_coordination",
            llm_prompt="Does supplier and vendor coordination exist with: supplier performance tracked, vendor relationships managed, coordination protocols established, and partnership value optimized? Return true if supplier and vendor coordination is effective, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cost_optimization_analysis",
            llm_prompt="Does cost optimization analysis exist with: cost drivers identified, optimization opportunities analyzed, cost reduction strategies implemented, and ROI measured? Return true if cost optimization analysis is complete and actionable, false otherwise.",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="route_convoy_planning_optimized",
            llm_prompt="Does optimized route and convoy planning exist with: route efficiency maximized, convoy coordination optimal, transportation costs minimized, and service levels maintained? Return true if route and convoy planning are optimized, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="capacity_buffer_management",
            llm_prompt="Does capacity buffer management exist with: safety stock levels optimized, buffer allocation strategic, capacity flexibility maintained, and service level targets met? Return true if capacity buffer management is effective, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="seasonality_historical_analysis",
            llm_prompt="Does seasonality and historical analysis exist with: 24-36 months data analyzed, seasonal effects captured, historical patterns documented, and trend analysis completed? Return true if seasonality and historical analysis are complete, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="documentation_procedures_maintained",
            llm_prompt="Do maintained documentation and procedures exist with: process documentation current, procedures standardized, knowledge management active, and operational guides accessible? Return true if documentation and procedures are maintained, false otherwise.",
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cross_lane_optimization",
            llm_prompt="Does cross-lane optimization exist with: lane performance compared, optimization opportunities identified across lanes, resource allocation balanced, and synergies captured? Return true if cross-lane optimization is implemented, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="stakeholder_communication_active",
            llm_prompt="Does active stakeholder communication exist with: internal communication protocols established, external partner communication active, status reporting regular, and alignment maintained? Return true if stakeholder communication is active and effective, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
