...
def create_preferences() -> PreferenceWeights:
            ...
            if any(
                ...
                if any(
                    (
                        ...
                        for keyword in ["approved", "validated", "certified", "audited", "reviewed"]
                    )
                ):
    ...
    def ai_safety_seed_rule(workflow: Workflow) -> float:
        ...
        keywords = ("safety_seed", "red_team_config", "safety_threshold", "test_reproducible")
        total = 0
        hits = 0
        for res in workflow.resources.values():
            total += 1
    ...
    quality_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt="Evaluate red team scenario coverage with specific evidence requirements:\n                - shows systematic prompt injection testing with documented failure rates\n                - demonstrates jailbreak resistance across multiple attack methodologies\n                - shows data exfiltration prevention with quantitative validation\n                - demonstrates unsafe function calling protection with test scenarios\n                - shows adversarial input handling under stress conditions\n                PENALTY: Deduct 2 points for each attack vector without quantitative results. No credit for theoretical coverage. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="ai_risk_type_coverage",
            llm_prompt="Score 0–10 for AI risk type coverage: safety violations, bias/fairness, hallucination, privacy leakage, adversarial attacks; award partial credit with testing methodology summaries. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="monitoring_systems_linkage",
            llm_prompt="Evaluate linkage of monitoring systems to safety results: alert thresholds, automated responses, escalation procedures. Award partial credit with citations. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="safety_testing_completeness",
            llm_prompt="Assess safety testing completeness (including human adversarial testing). Partial credit across scenarios, automation, and remediation tracking. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
    ...
    compliance_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Evaluate mapping of AI feature to privacy regulations (GDPR/CCPA/AI Act). Award partial credit for: coverage, evidence quality (citations), AI-specific compliance rationale, and explicit gaps/limitations. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="stakeholder_signoffs_present",
            llm_prompt="Assess formal sign‑offs: named approvers (Product/Security/Privacy/Legal/Executive) with dates and scope; reference specific AI safety sections. Award partial credit. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="dpia_data_lineage_audit_trail",
            llm_prompt="Evaluate DPIA and data lineage auditability: data flow registries, consent mechanisms, DSR handling, reproducibility steps. Award partial credit with citations. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="ai_model_risk_controls",
            llm_prompt="Assess AI model risk controls: bias monitoring, performance validation, safety threshold enforcement, model version control. Provide partial credit with citations. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="pii_sensitive_info_handling",
            llm_prompt="Evaluate PII and sensitive information handling: detection systems, data minimization, anonymization procedures, access controls. Cite evidence and award partial credit. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="ai_transparency_disclosure_requirements",
            llm_prompt="Assess AI transparency disclosure compliance: user notifications, capability limitations, model cards, content labeling implementation. Award partial credit for regulatory alignment. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="external_compliance_validation",
            llm_prompt="Evaluate external compliance validation and adversarial pressure handling:\n                - shows external legal or regulatory review of AI compliance approach\n                - demonstrates third-party security audit of AI safety controls\n                - shows preparation for regulatory investigation or compliance challenge\n                - demonstrates handling of public bias or safety complaints\n                - shows independent validation of privacy and data protection measures\n                PENALTY: Deduct 2 points for each missing external validation. No credit for internal-only reviews. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
    governance_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Evaluate AI governance framework maturity: decision rights clarity, approval workflows, risk escalation procedures, accountability assignment. Award partial credit for documented processes. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="launch_gate_governance_effectiveness",
            llm_prompt="Assess launch gate governance: safety thresholds, approval criteria, gate progression logic, rollback triggers. Award partial credit for clear criteria and evidence. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="cross_functional_coordination_quality",
            llm_prompt="Evaluate cross-functional coordination: stakeholder engagement, dependency management, communication effectiveness, conflict resolution. Cite coordination evidence. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="incident_response_governance",
            llm_prompt="Assess incident response governance: escalation procedures, decision authority, communication protocols, kill switch governance. Award partial credit for preparedness evidence. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
        ...
        WorkflowRubric(
            ...
            llm_prompt="Evaluate audit readiness: evidence linkage, decision traceability, compliance documentation completeness, regulatory scrutiny preparation. Award partial credit for audit trail quality. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
...
def create_evaluator_to_measure_goal_achievement() -> Evaluator:
    ...
    goal_achievement_rubrics = [
        WorkflowRubric(
            ...
            llm_prompt="Does a complete security assessment exist with: threat analysis, vulnerability assessment, security controls, and formal approval sign-off? Return 10.0 if all components exist and approved, removing 2.0 for every missing component to a minimum of 0.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="legal_compliance_verified",
            llm_prompt="Does verified legal compliance documentation exist with: regulatory review, privacy impact assessment, terms of service updates Return 10.0 if all verified and approved, removing 4.0 for every missing component.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="production_deployment_ready",
            llm_prompt="Does production deployment readiness exist with: deployment procedures, rollback plans, infrastructure provisioning, and ops team sign-off? Return 10.0 if fully ready for production, 5 if there is a evidence of a clear plan with progress made but it is not complete, 0.0 otherwise.",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="product_specification_exists",
            llm_prompt="Does a product specification document exist with: feature requirements, user stories, acceptance criteria, and technical constraints? Return 7.0 if all components exist, removing 2.0 for every missing component to a minimum of 0.0.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="technical_architecture_documented",
            llm_prompt="Does technical architecture documentation exist with: system design, infrastructure requirements, API specifications, and integration points? Return 7.0 if comprehensive documentation exists, removing 2.0 for every missing component to a minimum of 0.0.",
            max_score=7.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="performance_benchmarks_established",
            llm_prompt="Do performance benchmarks exist with: baseline metrics, target thresholds, measurement methodology, and success criteria? Return 5.0 if complete benchmarks exist, removing 1.0 for every missing component to a minimum of 0.0.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="user_acceptance_criteria_defined",
            llm_prompt="Do user acceptance criteria exist with: success metrics, user scenarios, testing procedures, and acceptance thresholds? Return true if complete criteria exist, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="launch_timeline_with_milestones",
            llm_prompt="Does a launch timeline exist with: key milestones, dependencies, critical path, resource allocation, and delivery dates? Return true if detailed timeline exists, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="success_metrics_established",
            llm_prompt="Do success metrics exist with: KPI definitions, measurement methods, target values, monitoring procedures, and reporting framework? Return true if complete metrics framework exists, false otherwise.",
            max_score=5.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="development_environment_setup",
            llm_prompt="Does development environment setup exist with: development tools, testing frameworks, CI/CD pipelines, and developer documentation? Return 3.0 if environment if evidence of a clear plan with progress made, 0.0 otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="user_documentation_complete",
            llm_prompt="Does user documentation exist with: user guides, API documentation, troubleshooting guides, and support procedures? Return true if comprehensive documentation exists, false otherwise.",
            max_score=3.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
