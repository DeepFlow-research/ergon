...
def deadtime_efficiency_score(workflow: Workflow, context: ValidationContext) -> Tuple[float, str]:
...
def build_operational_efficiency_evaluator() -> Evaluator:
    rubrics: list[WorkflowRubric] = [
        WorkflowRubric(
            ...
            evaluator_function=coordination_deadtime_score,
            max_score=1.0,
        ...
        WorkflowRubric(
            ...
            evaluator_function=deadtime_efficiency_score,
            max_score=1.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
