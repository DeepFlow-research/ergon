        ...
        rubrics.extend(
            [
                WorkflowRubric(
                    ...
                    llm_prompt="Hard constraint: OCC application package submitted with trackable artifact.\nReturn 1 if a submission artifact exists; else 0.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                ),
                WorkflowRubric(
                    name="pii_secrets_redaction",
                    llm_prompt="Hard constraint: No secrets or PII in shared artifacts.\nIf any secret/PII terms are present, return 0; else 1.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                ),
            ]
        ...
        rubrics.extend(
            [
                WorkflowRubric(
                    ...
                    llm_prompt="Hard constraint: At least one model card and evaluation report artifact must exist before deployment.\nReturn 1 if present; else 0.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                ),
                WorkflowRubric(
                    name="data_lineage_registry_initialized",
                    llm_prompt="Hard constraint: Data lineage/registry is initialized with versioned datasets.\nReturn 1 if present; else 0.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                ),
            ]
        ...
        rubrics.extend(
            [
                WorkflowRubric(
                    ...
                    llm_prompt="Hard constraint: A marked-up draft (redlines or tracked changes) must exist before negotiation credit.\nReturn 1 if present; else 0.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                )
            ]
        ...
        rubrics.extend(
            [
                WorkflowRubric(
                    ...
                    llm_prompt="Hard constraint: Jurisdictional statutory notification windows must be met.\nReturn 1 if artifacts show on-time notices; else 0.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                )
            ]
        ...
        rubrics.extend(
            [
                WorkflowRubric(
                    ...
                    llm_prompt="Hard constraint: Supplier QA certifications must be present before go-live.\nReturn 1 if certifications exist; else 0.",
                    max_score=1.0,
                    run_condition=RunCondition.ON_COMPLETION,
                )
            ]
