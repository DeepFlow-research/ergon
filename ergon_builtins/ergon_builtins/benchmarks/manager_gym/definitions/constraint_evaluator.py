...
def hard_constraints_enforced(workflow: Workflow, context: ValidationContext) -> Tuple[float, str]:
...
def constraint_coverage_mapping(
    ...
    for c in workflow.constraints:
        ...
        tasks_match = any(
            (
                name_l in t.name.lower() or any((tp.lower() in t.name.lower() for tp in types_))
                for t in workflow.tasks.values()
            )
        )
        resources_match = any(
...
def deadline_guardrails(workflow: Workflow, context: ValidationContext) -> Tuple[float, str]:
...
def build_constraint_evaluator() -> Evaluator:
    rubrics: list[WorkflowRubric] = [
        WorkflowRubric(
            ...
            description="LLM check: 1 if all hard constraints satisfied; 0 if any violation.",
        ...
        WorkflowRubric(
            ...
            evaluator_function=constraint_coverage_mapping,
            max_score=1.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            evaluator_function=deadline_guardrails,
            max_score=1.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            evaluator_function=prohibited_actions_avoidance,
            max_score=1.0,
            run_condition=RunCondition.ON_COMPLETION,
        ...
        WorkflowRubric(
            ...
            llm_prompt="Evaluate access control and PII handling evidence: redactions, RBAC documentation, and access logs. Provide partial credit and cite artifacts. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="formal_signoffs_present",
            llm_prompt="Assess presence and completeness of formal sign-offs (names, dates, scope, references). Provide partial credit and cite artifacts. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="data_lineage_controls_evidence",
            llm_prompt="Evaluate data lineage and controls: registry, reconciliations, reproducibility, and remediation logs. Provide partial credit and cite artifacts. Output numeric score [0, 10].",
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            name="soft_constraints_tradeoff_documentation",
            llm_prompt="When soft constraints are not strictly met, assess whether rationale/trade-offs are documented in artifacts and communications. Provide partial credit and cite. Output numeric score [0, 6].",
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
    ]
