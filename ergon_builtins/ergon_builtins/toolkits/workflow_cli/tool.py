...
class WorkflowCommandExecutor(Protocol):
    ...
    ) -> WorkflowCommandOutput | Awaitable[WorkflowCommandOutput]: ...
    ...
    *,
    ...
    async def run_command(command: str) -> str:
        ...
        if inspect.isawaitable(maybe_output):
            output = await cast(Awaitable[WorkflowCommandOutput], maybe_output)
        else:
            output = cast(WorkflowCommandOutput, maybe_output)
    ...
    if budgeted:
        ...
        async def workflow(
            ...
            """Inspect workflow topology/resources or add object-bound subtasks."""
