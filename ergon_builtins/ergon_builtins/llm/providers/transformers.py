...
class TransformersModel(_models.Model):
    ...
    def __init__(
        ...
        *,
        ...
        self._hf_model: Any = None  # slopcop: ignore[no-typing-any]
        self._hf_tokenizer: Any = None  # slopcop: ignore[no-typing-any]
        self._outlines_model: Any = None  # slopcop: ignore[no-typing-any]
    ...
    def _ensure_loaded(self) -> None:
        ...
        self._hf_model = model.to(self._device)
        self._hf_model.eval()
    ...
    def _generate_unconstrained(self, prompt_text: str) -> str:
        ...
        return cast("str", self._hf_tokenizer.decode(generated_ids, skip_special_tokens=True))
    ...
    def _generate_constrained(self, prompt_text: str, json_schema: str) -> str:
        ...
        try:
            ...
            logger.debug("Constrained generation produced %d chars", len(result_text))
            return result_text
        except ImportError:
        ...
        for msg in messages:
            if isinstance(msg, _messages.ModelRequest):
                for part in msg.parts:
                    if chat_message := self._request_part_to_chat_message(part):
                        chat_messages.append(chat_message)
            elif isinstance(msg, _messages.ModelResponse):
                for part in msg.parts:
                    if chat_message := self._response_part_to_chat_message(part):
                        chat_messages.append(chat_message)
    ...
    def _request_part_to_chat_message(
        self,
        part: _messages.ModelRequestPart,
        ...
        if isinstance(part, _messages.ToolReturnPart):
            ...
            return {"role": "tool", "content": content, "tool_call_id": part.tool_call_id}
        return None

    def _response_part_to_chat_message(
        self,
        part: _messages.ModelResponsePart,
    ) -> dict[str, object] | None:
