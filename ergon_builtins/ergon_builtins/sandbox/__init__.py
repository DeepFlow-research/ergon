"""Cross-cutting sandbox infrastructure shared across environments.

Files in this package implement adapters / utilities that more than
one environment's `Sandbox` subclass relies on.  Environment-specific code
(`LeanSandbox`, `SWEBenchSandbox`, etc.) lives in
`ergon_builtins/benchmarks/<slug>/sandbox.py`, NOT here. E2B-backed
environments share ``E2BSandbox`` and ``E2BSandboxRuntime`` from this package.

Naming: singular ``sandbox/`` (this package, cross-cutting infra)
vs. environment-specific ``benchmarks/<slug>/sandbox.py`` (single concrete
